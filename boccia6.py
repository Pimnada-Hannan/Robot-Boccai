...
class FieldMeasureApp:
    ...
    TARGET_RADIUS = 2
    WHITE_COLOR_LOWER = np.array([100, 100, 200], np.uint8)
    ...
    def create_widgets(self):
        ...
        Button(
            self.control_frame,
            text="Show White Ball Mask",
            command=self.show_white_ball_mask,
            bg="lightblue",
            font=("Arial", 10, "bold"),
    ...
    def update_camera(self):
        while self.cap is not None and self.cap.isOpened() and self.running:
            ...
            if ret:
                ...
                # แสดงภาพขาวดำสำหรับลูกบอลสีขาว
                self.show_white_ball_mask()
