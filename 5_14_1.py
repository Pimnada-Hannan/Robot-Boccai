...
class FieldMeasureApp:
    ...
    def create_widgets(self):
        ...
        Label(
            self.control_frame, text="Set Target Position", font=("Arial", 12, "bold")
        ).grid(row=25, column=0, columnspan=2, pady=5)

        Label(self.control_frame, text="X (cm):").grid(
            row=26, column=0, sticky="w", padx=5
        )
        self.x_entry = Entry(self.control_frame, width=10)
        self.x_entry.grid(row=26, column=1, sticky="w", padx=5)

        Label(self.control_frame, text="Y (cm):").grid(
            row=27, column=0, sticky="w", padx=5
        )
        self.y_entry = Entry(self.control_frame, width=10)
        self.y_entry.grid(row=27, column=1, sticky="w", padx=5)

        # ปุ่มสำหรับยืนยันค่าที่ป้อน
        Button(
            self.control_frame,
            text="Set Position",
            command=self.set_target_position,
            bg="lightblue",
            font=("Arial", 10, "bold"),
        ).grid(row=28, column=0, columnspan=2, pady=10, sticky="ew")
        ...
        # ==== Corner Points Listbox ====
        Label(
            self.control_frame, text="Corner Points", font=("Arial", 12, "bold")
        ).grid(row=29, column=0, columnspan=2, pady=5)
        self.corner_listbox = Listbox(self.control_frame, height=5, width=30)
        self.corner_listbox.grid(row=30, column=0, columnspan=2, sticky="ew", padx=5)

        Button(
            self.control_frame, text="Remove Last Point", command=self.remove_last_point
        ).grid(row=31, column=0, columnspan=2, sticky="ew", pady=2)

        Button(self.control_frame, text="Clear Points", command=self.clear_points).grid(
            row=32, column=0, columnspan=2, sticky="ew", pady=2
        )
    ...
    def detect_balls_in_frame(self):
        ...
        # ===================== WHITE BALL DETECTION WITH HSV ===================== #
        hsv_img = cv2.cvtColor(self.img, cv2.COLOR_BGR2HSV)
        white_mask_raw = cv2.inRange(
            hsv_img, self.WHITE_COLOR_LOWER, self.WHITE_COLOR_UPPER
        )
        white_mask = cv2.bitwise_and(
            white_mask_raw, white_mask_raw, mask=mask
        )  # Apply field mask
        ...
        # ===================== COLOR BALL DETECTION (LAB CLAHE) ===================== #
        ...
        color_ranges = {
            ...
            "blue": [
                (np.array([100, 150, 30]), np.array([130, 255, 80])),
            ],
        }
                ...
                if 200 < area < 1500:
                    ...
                    if circularity > self.BALL_CIRCULARITY_THRESHOLD:
                        ...
                        if solidity > self.BALL_SOLIDITY_THRESHOLD:
                            ...
                            x2, y2 = int(x + radius), int(y + radius)
                            confidence = min(1.0, (circularity * 0.7 + solidity * 0.3))
                            cls_id = 2 if color_name == "red" else 0
        ...
        # ===================== NMS MERGING ALL ===================== #
        all_detections = white_detections + color_detections
        boxes = [
            [x1, y1, x2 - x1, y2 - y1]
            for x1, y1, x2, y2 in (d["bbox"] for d in all_detections)
        ]
        ...
        self.ball_all = [all_detections[i] for i in nms_indices] if nms_indices else []

        # ===================== SELECT BEST WHITE BALL ===================== #
        white_balls = [d for d in self.ball_all if d["class"] == 1]
        if white_balls:
    ...
    def set_target_position(self):
        try:
            ...
            print(f"Target Position Set: X = {x_value} cm, Y = {y_value} cm")
            # คุณสามารถใช้ค่าที่ป้อนไปทำงานต่อได้ เช่น เก็บในตัวแปร
            self.target_position = (x_value, y_value)
        except ValueError:
    ...
    def process_measurements(self):
        ...
        if len(self.field_pts) == 4 and self.ball_pt1:
            ...
            # ดึงค่าจาก target_position
            if hasattr(self, "target_position"):
                x2_cm, y2_cm = self.target_position
            else:
                messagebox.showerror("Error", "Target position not set.")
                return
            ...
            pymc3e.connect(ip="192.168.0.200", port=2001)
            ...
            pymc3e.randomwrite(
                ...
                word_values=[distance_cm, angle_degrees],
                dword_devices=[],
                dword_values=[],
            )
            pymc3e.settimeout(5)  # ตั้ง Timeout เป็น 5 วินาที
...
# ใช้ iQ-R หรือปรับตามรุ่น PLC ของคุณ
# pymc3e = pymcprotocol.Type3E(plctype="iQ-R")

# # ไม่ต้องตั้ง commtype ถ้าใช้ binary (default)
# # pymc3e.setaccessopt(commtype="ascii")  ← ไม่ใส่

# # เชื่อมต่อ PLC
# pymc3e.connect("192.168.0.200", 2001)
# pymc3e.setaccessopt(commtype="ascii")

# เขียนค่าระยะทางและองศา
# pymc3e.randomwrite(
#     word_devices=["D50", "D55"],
#     word_values=[distance_cm, angle_degrees],
# )
