...
class CalibrationWindow(Toplevel):
    ...
    def setup_color_controls(self, parent_frame, color_name):
        ...
        right_panel = Frame(parent_frame, width=self.preview_size[0] + 10)
    ...
    def reset_current_tab_to_defaults(self):
        try:
            ...
            if color_name and color_name in self.main_app.DEFAULT_DETECTION_PARAMS["colors"]:
                ...
                self.load_params_to_ui()
                self.refresh_mask_display(color_name)
                if color_name == "white": self.refresh_all_mask_displays()
...
class FieldMeasureApp:
    ...
    OPPONENT_NEAR_JACK_THRESHOLD_CM = 30.0
    TEAM_NONE=0; TEAM_RED=1; TEAM_BLUE=2

    def __init__(self, root_tk):
        self.root = root_tk
        self.root.title("Field Measurement Tool v6.2 - Red Jack Throw")
    ...
    def create_widgets(self):
        ...
        Label(f_target,text="Pulse Offset:",font=sfont_small).grid(row=2,column=0,sticky=W,padx=2)
        Entry(f_target,width=7,textvariable=self.pulse_offset_var,font=sfont_small).grid(row=2,column=1,sticky=W,padx=2)
        ...
        Button(f_target,text="Calc & Show Detail",command=lambda:self.run_full_detection_cycle(True),bg="lightgreen",font=sfont_bold).grid(row=4,column=0,columnspan=2,pady=1,sticky="ew",ipady=0) # Was row 3
        self.distance_display_label=Label(f_target,textvariable=self.distance_display_str,font=("Arial",9,"bold")); self.distance_display_label.grid(row=5,column=0,columnspan=2,pady=0,sticky="w") # Was row 4
        self.angle_display_label=Label(f_target,textvariable=self.angle_display_str,font=("Arial",9,"bold")); self.angle_display_label.grid(row=6,column=0,columnspan=2,pady=0,sticky="w") # Was row 5
        f_target.grid_columnconfigure(1,weight=1)

        f_plc = LabelFrame(self.control_frame, text="PLC Status", font=sfont_bold)
        f_plc.pack(fill=X, padx=2, pady=(5,1), side=BOTTOM)
    ...
    def _dynamic_scale_and_set_preview(self):
        ...
        info_h_est=self.LOUPE_DIM+10; coord_h_est=20 # LOUPE_DIM is now 240
                            ...
                            if len(self.field_pts) == 4:
                                ...
                                if jack_cm_for_eval and dist_for_eval is not None and angle_for_eval is not None:
                                    ...
                                    if opponent_near_jack:
                                        ...
                                        final_swing_speed = (dist_for_eval * 24.096) + 5900 +pulse_offset_val
                                        final_swing_speed = max(0, min(final_swing_speed, 23000))
                                        final_release_speed = 500
                                        # Angle adjustment logic for opponent_near_jack (if any) could be here or combined
            ...
            if self.ball_detected and self.ball_pt1 is not None:
                ...
                if measurement_data_dict:
                    ...
                    if opponent_near_jack: # This formula is different, not affected by pulse_offset
                        ...
                        final_swing_speed = (final_dist_cm * 24.096) + 5900  +10000 + pulse_offset_val
                        final_release_speed = 500
                        m190_value_to_set = 1
...
if __name__ == "__main__":
    root = Tk()
    root.geometry("1000x920") # Window size might need adjustment depending on new LOUPE_DIM
