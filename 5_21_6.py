...
class FieldMeasureApp:
    ...
    DEFAULT_DETECTION_PARAMS = {  # From boccia16.py
        ...
        "colors": {
            "white": {
                "hsv_ranges": [(np.array([0, 0, 150]), np.array([180, 65, 255]))],
                "circularity": 0.65,
                "solidity": 0.75,
                "area_min": 100,
                "area_max": 700,
            ...
            while self.running:
                ...
                if ret and frame is not None:
                    ...
                    if (
                        ...
                        break
                    self._dynamic_scale_and_set_preview()
                    self.detect_balls_in_frame()  # This sets ball_all, ball_pt1 and also self.current_hsv_combined_mask_display
    ...
    # # Add a button to the control_frame to call red_team

    def red_team(self):
        try:
            pymc3e = pymcprotocol.Type3E()
            wordunits_values = pymc3e.batchread_wordunits(headdevice="M300", readsize=2)

            if wordunits_values[0] == 1:
                # Create a popup window for user input
                popup = Toplevel(self.root)
                popup.title("Red Team Manual Input")
                popup.geometry("300x200")
                popup.grab_set()

                Label(popup, text="Enter Distance:").pack(pady=5)
                distance_entry = Entry(popup)
                distance_entry.pack(pady=5)

                Label(popup, text="Enter Degree:").pack(pady=5)
                degree_entry = Entry(popup)
                degree_entry.pack(pady=5)

                def submit():
                    try:
                        distance1 = float(distance_entry.get())
                        degrees1 = float(degree_entry.get())
                        ...
                        messagebox.showinfo("Success", "Data sent to PLC.")
                        popup.destroy()
                    except ValueError:
                        messagebox.showerror(
                            "Invalid input",
                            "Please enter numeric values for distance and degree.",
                        )
                    except Exception as e:
                        messagebox.showerror("Error", f"An error occurred: {e}")

                Button(popup, text="Submit", command=submit, bg="lightgreen").pack(
                    pady=10
                )
                Button(popup, text="Cancel", command=popup.destroy, bg="salmon").pack(
                    pady=2
                )
            else:
                messagebox.showinfo("Info", "M300 is not set to 1.")
        except Exception as e:
            messagebox.showerror("Error", f"An error occurred: {e}")
