    ...
    def generate_mask_for_color(self, color_name, image_to_process):
        ...
        for lower_hsv_np, upper_hsv_np in hsv_ranges:
            ...
            if temp_upper[1] < temp_lower[1]:
                temp_upper[1] = temp_lower[1]
            if temp_upper[2] < temp_lower[2]:
                temp_upper[2] = temp_lower[2]
        ...
        dilate_k = params.get("morph_dilate_k", 5)
        dilate_iter = params.get("morph_dilate_iter", 1)
    ...
    def reset_current_tab_to_defaults(self):
        try:
            ...
            if (
                ...
                if (
                    color_name == "white"
                ):  # Also refresh all if white changes global blur
                    self.refresh_all_mask_displays()
...
class FieldMeasureApp:
    ...
    def __init__(self, root_tk):
        self.root = root_tk
        self.root.title(
            "Field Measurement Tool (5_21_3 - Path Obstacle)"
        )  # Updated title
        ...
        self.target_x_cm_str = StringVar(value="155.0")  # Default from 5_21_3.py
        self.target_y_cm_str = StringVar(value="750.0")  # Default from 5_21_3.py
    ...
    def create_widgets(self):
        ...
        # Red REd RED red reD
        red_team_frame = LabelFrame(
            self.control_frame, text="Red Team", font=("Arial", 10, "bold")
        )
        red_team_frame.pack(fill=X, padx=5, pady=5)

        Button(
            red_team_frame,
            text="เลือกตำแหน่ง Red Team (คลิกบนภาพ)",
        ...
        self.red_team_xy_label = Label(
            red_team_frame,
            textvariable=self.red_team_xy_var,
            font=("Arial", 12, "bold"),
            bg="white",
        )
        self.red_team_xy_label.pack(fill=X, pady=2)

        Button(
            red_team_frame,
            text="ส่งข้อมูล Red Team ไป PLC",
    ...
    def _is_obstacle_on_path(
        ...
        all_detected_balls,
        H_matrix,
        proximity_threshold_cm=8.7,
    ):
        if not all_detected_balls or H_matrix is None:
            return False
        p1_x, p1_y = jack_cm
        t_x, t_y = target_cm
        vec_p1_t_x = t_x - p1_x
        ...
        for ball_info in all_detected_balls:
            if ball_info["color_name"] in ["red", "blue"]:
                ball_pixel_center = np.float32([ball_info["center"]]).reshape(-1, 1, 2)
                ball_cm_transformed = cv2.perspectiveTransform(
                    ball_pixel_center, H_matrix
                )
                if ball_cm_transformed is None or ball_cm_transformed.size == 0:
                    continue
                rb_x, rb_y = ball_cm_transformed[0, 0]
                line_A = t_y - p1_y
                line_B = p1_x - t_x
                line_C = t_x * p1_y - t_y * p1_x
                denominator = math.sqrt(line_A**2 + line_B**2)
                if denominator == 0:
                    continue
                distance_to_line = (
                    abs(line_A * rb_x + line_B * rb_y + line_C) / denominator
                )
                if distance_to_line < proximity_threshold_cm:
                    vec_p1_rb_x = rb_x - p1_x
                    vec_p1_rb_y = rb_y - p1_y
                    dot_product = vec_p1_rb_x * vec_p1_t_x + vec_p1_rb_y * vec_p1_t_y
                    if 0 < dot_product < len_sq_p1_t:
                        # print(f"DEBUG: Obstacle {ball_info['color_name']} at ({rb_x:.1f}, {rb_y:.1f})cm is on path.")
                        return True
        return False
    ...
    def handle_canvas_click(self, event):
        if getattr(self, "selecting_red_team_point", False):
            ...
            self.canvas.config(cursor="")
            messagebox.showinfo("Red Team", f"เลือกตำแหน่งแล้ว: X={X}, Y={Y}")
        elif self.STATUS_CLICK_COLOR_INFO_MODE:
        ...
        try:
            while self.running:
                ...
                if ret and frame is not None:
                    ...
                    # PLC Read Logic from 5_21_3.py
                    ball_count = 0  # Default value
                    wordunits_values_m300 = 0  # Default value
                    try:
                        if self.plc_connected and self.pymc3e:
                            ball_count_read = self.pymc3e.batchread_wordunits(
                                headdevice="D130", readsize=1
                            )
                            wordunits_values_read = self.pymc3e.batchread_wordunits(
                                headdevice="M300", readsize=1
                            )
                            ball_count = ball_count_read[0] if ball_count_read else 0
                            wordunits_values_m300 = (
                                wordunits_values_read[0] if wordunits_values_read else 0
                            )
                    except Exception as plc_e:
                        print(f"PLC Read Error in camera_feed: {plc_e}")
                        # Keep default values if PLC read fails

                    # Conditional detection based on PLC values
                    if wordunits_values_m300 != 0:
                        if ball_count == 0:
                            if hasattr(self.root, "after") and self.root.winfo_exists():
                                self.root.after(0, self.red_team)
                            if hasattr(self.root, "after") and self.root.winfo_exists():
                                self.root.after(
                    ...
                    if len(self.field_pts) == 4:
                        if self.ball_detected and self.ball_pt1 is not None:
                            ...
                            if current_measurement_data:
                                H_matrix_local = current_measurement_data.get(
                                    "H_matrix"
                                )
                                if (
                                ...
                                if (
                                    H_matrix_local is not None and self.ball_all
                                ):  # CORRECTED CHECK
                                    jack_cm_coords = (
                                        current_measurement_data["x1_cm"],
                                        current_measurement_data["y1_cm"],
                                    )
                                    try:
                                        target_x = float(self.target_x_cm_str.get())
                                        target_y = float(self.target_y_cm_str.get())
                                        target_cm_coords = (target_x, target_y)
                                        ...
                                        print(
                                            "Warning: Invalid target coordinates for obstacle check in camera_feed."
                                        )
                                        # target_cm_coords will not be used if exception occurs, obstacle_on_path remains False

                                base_swing_offset = 5900
                                if obstacle_on_path:
                                    base_swing_offset = 6995
                    ...
                    if current_time - last_error_report_time > error_report_interval:
                        ...
                        # print(f"Camera feed: No frame received at {current_time}") # Optional debug
                    time.sleep(0.05)
                time.sleep(0.02)
    ...
    def run_full_detection_cycle(self, show_results_window=False):
        ...
        if len(self.field_pts) == 4:
            if self.ball_detected and self.ball_pt1 is not None:
                ...
                if measurement_data:
                    ...
                    obstacle_on_path = False
                    if H_matrix_local is not None and self.ball_all:  # CORRECTED CHECK
                        jack_cm_coords = (
                        ...
                        try:
                            ...
                            # target_cm_coords will not be used if exception occurs, obstacle_on_path remains False
                            print(
                                "Warning: Invalid target coordinates for obstacle check in run_full_detection_cycle."
                            )

                    base_swing_offset = 5900
                    if obstacle_on_path:
                        base_swing_offset = 6995
                        # print("RunFullDetect: Obstacle detected, using offset 6995")
            ...
            self.update_measurement_display_default()
            if (
                len(self.field_pts) != 4
                and not self.camera_mode
                and not show_results_window
            ):
        ...
        if show_results_window:
            ...
            if not self.ball_detected or self.ball_pt1 is None:
                ...
                return
            if (
                measurement_data is None
            ):  # Recalculate if somehow missed (e.g. if called directly and not from camera loop)
                measurement_data = self.process_measurements_for_realtime()
    ...
    def detect_balls_in_frame(self):
        if len(self.field_pts) != 4:
            ...
            if hasattr(self.root, "after") and self.root.winfo_exists():
                self.root.after(0, self.update_main_canvas_display)
            return
        ...
        white_color_params = self.active_detection_params["colors"].get("white")

        if white_color_params:
            hsv_ranges_list = white_color_params.get("hsv_ranges", [])
            if hsv_ranges_list:
                ...
                open_k = white_color_params.get("morph_open_k", 5)
                open_iter = white_color_params.get("morph_open_iter", 1)
                close_k = white_color_params.get("morph_close_k", 5)
                close_iter_val = white_color_params.get("morph_close_iter", 2)
                if open_k < 3:
                    open_k = 3
                if open_k % 2 == 0:
                    open_k += 1
                ...
                min_area = white_color_params.get("area_min", 100)
                max_area = white_color_params.get("area_max", 1500)
                circ_thresh = white_color_params.get("circularity", 0.65)
                solidity_thresh = white_color_params.get("solidity", 0.75)
                ...
                for cnt in contours_white:
                    ...
                    if circularity >= circ_thresh and solidity >= solidity_thresh:
                        ...
                        all_detected_objects_for_nms.append(candidate_ball)
                        primary_white_circularity_thresh = wp.get(
                            "primary_circularity", 0.65
                        )
                        primary_white_min_radius = wp.get("primary_min_radius", 6)
                        if (
                            circularity >= primary_white_circularity_thresh
                            and radius >= primary_white_min_radius
                        ):
                            metric = circularity * 1000 + radius
                            if metric > highest_primary_white_metric:
                                highest_primary_white_metric = metric
                                best_hsv_white_ball_for_primary = candidate_ball

        primary_center_x_px, primary_center_y_px, primary_radius_px = None, None, None
        if best_hsv_white_ball_for_primary:
            self.ball_detected = True
            primary_center_x_px, primary_center_y_px = best_hsv_white_ball_for_primary[
                "center"
            ]
            primary_radius_px = best_hsv_white_ball_for_primary["radius"]
            self.ball_pt1 = (
                primary_center_x_px,
                primary_center_y_px + primary_radius_px,
            )
            if hasattr(self, "ball_status_str"):
                self.ball_status_str.set(
                    f"W:Found (C:{best_hsv_white_ball_for_primary['circularity']:.2f} S:{best_hsv_white_ball_for_primary['solidity']:.2f} R:{primary_radius_px})"
                )

        if self.ball_detected and primary_center_x_px is not None:
            ROI_RADIUS_FACTOR = 12.0
            search_radius_sq = (primary_radius_px * ROI_RADIUS_FACTOR) ** 2

            for color_name in ["red", "blue"]:
                color_params = self.active_detection_params["colors"].get(color_name)
                if not color_params:
                    continue

                hsv_ranges_list_rb = color_params.get("hsv_ranges", [])
                if not hsv_ranges_list_rb:
                    continue

                current_color_binary_mask_detect_rb = np.zeros(
                    hsv_blurred.shape[:2], dtype=np.uint8
                )
                for lower_hsv_np, upper_hsv_np in hsv_ranges_list_rb:
                    temp_lower_det_rb = np.array(lower_hsv_np, dtype=np.uint8)
                    temp_upper_det_rb = np.array(upper_hsv_np, dtype=np.uint8)
                    if not (
                        color_name == "red"
                        and temp_lower_det_rb[0] > temp_upper_det_rb[0]
                    ):
                        if temp_upper_det_rb[0] < temp_lower_det_rb[0]:
                            temp_upper_det_rb[0] = temp_lower_det_rb[0]
                    if temp_upper_det_rb[1] < temp_lower_det_rb[1]:
                        temp_upper_det_rb[1] = temp_lower_det_rb[1]
                    if temp_upper_det_rb[2] < temp_lower_det_rb[2]:
                        temp_upper_det_rb[2] = temp_lower_det_rb[2]
                    individual_mask_segment_det_rb = np.zeros(
                        hsv_blurred.shape[:2], dtype=np.uint8
                    )
                    if (
                        color_name == "red"
                        ...
                        mask1 = cv2.inRange(
                            hsv_blurred,
                            np.array([0, temp_lower_det_rb[1], temp_lower_det_rb[2]]),
                            temp_upper_det_rb,
                        )
                        mask2 = cv2.inRange(
                            hsv_blurred,
                            temp_lower_det_rb,
                            np.array([179, temp_upper_det_rb[1], temp_upper_det_rb[2]]),
                        )
                        individual_mask_segment_det_rb = cv2.bitwise_or(mask1, mask2)
                    else:
                        individual_mask_segment_det_rb = cv2.inRange(
                            hsv_blurred, temp_lower_det_rb, temp_upper_det_rb
                        )
                    current_color_binary_mask_detect_rb = cv2.bitwise_or(
                        current_color_binary_mask_detect_rb,
                        individual_mask_segment_det_rb,
                    )

                open_k_rb = color_params.get("morph_open_k", 5)
                open_iter_rb = color_params.get("morph_open_iter", 1)
                close_k_rb = color_params.get("morph_close_k", 5)
                close_iter_val_rb = color_params.get("morph_close_iter", 1)
                dilate_k_rb = color_params.get("morph_dilate_k", 5)
                dilate_iter_rb = color_params.get("morph_dilate_iter", 1)
                if open_k_rb < 3:
                    open_k_rb = 3
                if open_k_rb % 2 == 0:
                    open_k_rb += 1
                ...
                if dilate_k_rb % 2 == 0:
                    dilate_k_rb += 1
                morph_mask_detect_rb = current_color_binary_mask_detect_rb
                if open_iter_rb > 0:
                    morph_mask_detect_rb = cv2.morphologyEx(
                        morph_mask_detect_rb,
                        cv2.MORPH_OPEN,
                        np.ones((open_k_rb, open_k_rb), np.uint8),
                        iterations=open_iter_rb,
                    )
                if dilate_iter_rb > 0:
                    morph_mask_detect_rb = cv2.dilate(
                        morph_mask_detect_rb,
                        np.ones((dilate_k_rb, dilate_k_rb), np.uint8),
                        iterations=dilate_iter_rb,
                    )
                if close_iter_val_rb > 0:
                    morph_mask_detect_rb = cv2.morphologyEx(
                        morph_mask_detect_rb,
                        cv2.MORPH_CLOSE,
                        np.ones((close_k_rb, close_k_rb), np.uint8),
                        iterations=close_iter_val_rb,
                    )

                contours_rb, _ = cv2.findContours(
                    morph_mask_detect_rb, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
                )
                min_area_rb = color_params.get("area_min", 100)
                max_area_rb = color_params.get("area_max", 700)
                circ_thresh_rb = color_params.get(
                    "circularity", 0.65 if color_name == "blue" else 0.7
                )
                solidity_thresh_rb = color_params.get("solidity", 0.65)

                for cnt_rb in contours_rb:
                    M = cv2.moments(cnt_rb)
                    if M["m00"] == 0:
                        continue
                    cx_rb = int(M["m10"] / M["m00"])
                    cy_rb = int(M["m01"] / M["m00"])
                    dist_sq = (cx_rb - primary_center_x_px) ** 2 + (
                        cy_rb - primary_center_y_px
                    ) ** 2
                    if dist_sq > search_radius_sq:
                        continue
                    area_rb = cv2.contourArea(cnt_rb)
                    if not (min_area_rb <= area_rb <= max_area_rb):
                        continue
                    perimeter_rb = cv2.arcLength(cnt_rb, True)
                    if perimeter_rb == 0:
                        continue
                    circularity_rb = 4 * np.pi * area_rb / (perimeter_rb * perimeter_rb)
                    (x_rb, y_rb), radius_rb = cv2.minEnclosingCircle(cnt_rb)
                    hull_rb = cv2.convexHull(cnt_rb)
                    hull_area_rb = cv2.contourArea(hull_rb)
                    solidity_rb = (
                        float(area_rb) / hull_area_rb if hull_area_rb > 0 else 0
                    )
                    if (
                        circularity_rb >= circ_thresh_rb
                        and solidity_rb >= solidity_thresh_rb
                    ):
                        class_id = {"red": 2, "blue": 0}.get(color_name, -1)
                        confidence_rb = (
                            circularity_rb * 0.5
                            + solidity_rb * 0.3
                            + min(
                                1.0,
                                area_rb
                                / (max_area_rb if max_area_rb > 0 else area_rb + 1),
                            )
                            * 0.2
                        )
                        candidate_ball_rb = {
                            ...
                            "radius": int(radius_rb),
                            "color_name": color_name,
                            "circularity": circularity_rb,
                            "solidity": solidity_rb,
                            "area": area_rb,
                        }
                        all_detected_objects_for_nms.append(candidate_ball_rb)
        ...
        if all_detected_objects_for_nms:
            ...
            confidences_for_nms = []
            original_indices = []
            for i, det_obj in enumerate(all_detected_objects_for_nms):
                x1, y1, x2, y2 = det_obj["bbox"]
                boxes_for_nms.append([x1, y1, x2 - x1, y2 - y1])
                confidences_for_nms.append(det_obj["confidence"])
                ...
                if isinstance(nms_indices, np.ndarray):
                    ...
                    for index in nms_indices:
                        self.ball_all.append(
                            all_detected_objects_for_nms[original_indices[index]]
                        )
        if self.ball_detected and best_hsv_white_ball_for_primary:
            is_primary_in_final_list = any(
                b["center"] == best_hsv_white_ball_for_primary["center"]
                and b["color_name"] == "white"
                for b in self.ball_all
            )
            if not is_primary_in_final_list:
                self.ball_all.append(best_hsv_white_ball_for_primary)
        if not self.ball_all and self.ball_detected and best_hsv_white_ball_for_primary:
            self.ball_all.append(best_hsv_white_ball_for_primary)
        if not self.ball_all and not self.ball_detected:
            if hasattr(
                self, "ball_status_str"
    ...
    def update_main_canvas_display(self):
        if self.preview is None:
            ...
            if (
                ...
                blank_mask_rgb = cv2.cvtColor(blank_mask_for_tk, cv2.COLOR_BGR2RGB)
        ...
        for det_ball in self.ball_all:
            ...
            text_x = center_prev[0] - radius_prev // 2
            text_y = center_prev[1] - radius_prev - 5
            cv2.putText(
                preview_to_draw_on,
                label_text,
                (text_x, text_y),
        ...
        if self.ball_detected and self.ball_pt1:
            p1_marker_x = int(self.ball_pt1[0] * self.PREVIEW_SCALE)
            p1_marker_y = int(self.ball_pt1[1] * self.PREVIEW_SCALE)
            cv2.drawMarker(
                preview_to_draw_on,
                (p1_marker_x, p1_marker_y),
                (255, 255, 0),
        ...
        if (
            ...
            try:
                ...
                mask_rgb_for_tk = cv2.cvtColor(mask_resized_for_tk, cv2.COLOR_BGR2RGB)
    ...
    def process_measurements_for_realtime(self):
        ...
        return {
            ...
            "H_matrix": H_matrix,  # Return H_matrix
        }
        ...
        if len(self.field_pts) == 4:
            if self.ball_detected and self.ball_pt1 is not None:
                ...
                if measurement_data:
                    ...
                    obstacle_on_path = False
                    if H_matrix_local is not None and self.ball_all:
                        jack_cm_coords = (
                            measurement_data["x1_cm"],
                            measurement_data["y1_cm"],
                        )
                        try:
                            ...
                            target_cm_coords = (0.0, 0.0)  # Fallback
                            print(
                                "Warning: Invalid target coordinates for obstacle check in run_full_detection_cycle."
                            )
    ...
    def show_detailed_results_window(self, measurement_data):
        ...
        H_matrix = measurement_data.get("H_matrix")
        if any(
            v is None
            for v in [
                x1_cm,
    ...
    def start_red_team_select_mode(self):
        ...
        messagebox.showinfo("Red Team", "คลิกบนภาพเพื่อเลือกตำแหน่ง Red Team")
        self.canvas.config(cursor="crosshair")
    ...
    def red_team(self):
        ...
        try:
            # อ่านค่าจาก PLC
            wordunits_values = self.pymc3e.batchread_wordunits(
                headdevice="M300", readsize=2
            )
            ...
            if wordunits_values[0] == 3:
                ...
                ball_count = self.pymc3e.batchread_wordunits(
                    headdevice="D130", readsize=2
                )

                if ball_count[0] == 0:
                    print(ball_count[0])

                    # ใช้จุดที่ผู้ใช้คลิกบนสนาม (field_pts) แทนการกรอกข้อมูล
                    # ให้ผู้ใช้คลิก 1 จุดบน preview เพื่อเลือกตำแหน่ง Red Team
                    # จะใช้ self.red_team_selected_point ซึ่งต้องกำหนดใน handle_canvas_click

                    if (
                        not hasattr(self, "red_team_selected_point")
                        or self.red_team_selected_point is None
                    ):
                        messagebox.showwarning(
                            "Red Team", "กรุณาคลิกเลือกตำแหน่ง Red Team บนภาพก่อน"
                        )
                        return

                    X, Y = self.red_team_selected_point
...
if __name__ == "__main__":
    root = Tk()
    root.geometry("1500x1250")  # From 5_21_3.py
