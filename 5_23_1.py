...
class CalibrationWindow(Toplevel):
    ...
    def setup_color_controls(self, parent_frame, color_name):
        ...
        right_panel = Frame(
            parent_frame, width=self.preview_size[0] + 10
        )  # Adjusted width
        ...
        morph_ops = [
            ("morph_open_k", "Open K (odd,>=3):", 3, 21, 2),
            ("morph_open_iter", "Open Iter:", 0, 5, 1),
            ("morph_close_k", "Close K (odd,>=3):", 3, 21, 2),
            ("morph_close_iter", "Close Iter:", 0, 5, 1),
        ]
        if color_name in ["red", "blue"]:
            morph_ops.extend(
                [
                    ("morph_dilate_k", "Dilate K (odd,>=3):", 3, 21, 2),
                    ("morph_dilate_iter", "Dilate Iter:", 0, 5, 1),
                ]
            )
        for r_idx, (param_key, text, from_val, to_val, res) in enumerate(morph_ops):
            Label(morph_frame, text=text).grid(
                row=r_idx, column=0, sticky=W, padx=1, pady=1
            )
            scale = Scale(
                morph_frame,
                from_=from_val,
        ...
        detection_params_frame = LabelFrame(
            left_panel, text="Detection Params", padx=3, pady=3
        )
        detection_params_frame.pack(fill=X, pady=2)

        # Entries for area
        det_entries_config = [
            ("area_min", "Min Area:", int),
            ("area_max", "Max Area:", int),
        ]
        if color_name in ["red", "blue"]:
            det_entries_config.extend(
                [
                    ("cluster_area_min", "Min Cluster Area:", int),
                    ("cluster_area_max", "Max Cluster Area:", int),
                ]
            )
        ...
        for param_key, text, type_func in det_entries_config:
            Label(detection_params_frame, text=text).grid(
                row=current_row, column=0, sticky=W, padx=1, pady=1
            )
        ...
        det_scales_config = [
            ("circularity", "Circularity (0-100%):", 100.0),
            ("solidity", "Solidity (0-100%):", 100.0),
        ]
        if color_name in ["red", "blue"]:
            det_scales_config.extend(
                [
                    (
                        "cluster_min_aspect_ratio",
                        "Min Clust. Aspect (0-100%):",
                        100.0,
                    ),  # % of 1.0
                    (
                        "cluster_max_aspect_ratio",
                        "Max Clust. Aspect (0-500%):",
                        100.0,
                    ),  # % of 1.0, e.g. 300 for 3.0
                ]
            )
        ...
        self.color_vars[f"{color_name}_mask_photo"] = (
            None  # To hold PhotoImage reference
        )
    ...
    def load_params_to_ui(self):
        ...
        if blur_k_val % 2 == 0:
            blur_k_val = max(3, blur_k_val - 1 if blur_k_val > 0 else 3)
        if (
        ...
        for color_name in ["white", "red", "blue"]:
            ...
            if color_name in ["red", "blue"]:
                morph_keys.extend(["morph_dilate_k", "morph_dilate_iter"])
            for pk in morph_keys:
                if (
                    f"{pk}_scale" in self.color_vars[color_name]
                    and self.color_vars[color_name][f"{pk}_scale"].winfo_exists()
                ):
                    val = color_data.get(
                        pk, default_color_data.get(pk, 3 if "_k" in pk else 1)
                    )
                    if "_k" in pk and val % 2 == 0:
                        val = max(3, val - 1 if val > 0 else 3)
                    self.color_vars[color_name][f"{pk}_scale"].set(val)

            entry_pks = ["area_min", "area_max"]
            if color_name in ["red", "blue"]:
                entry_pks.extend(["cluster_area_min", "cluster_area_max"])
            for pk_entry in entry_pks:
                if (
                    f"{pk_entry}_entry" in self.color_vars[color_name]
                    and self.color_vars[color_name][f"{pk_entry}_entry"].winfo_exists()
                ):
                    self.color_vars[color_name][f"{pk_entry}_entry"].delete(0, END)
                    def_val = default_color_data.get(
                        pk_entry,
                        (
                            100
                            if "min" in pk_entry
                            else (700 if "cluster" not in pk_entry else 5000)
                        ),
                    )
                    self.color_vars[color_name][f"{pk_entry}_entry"].insert(
                        0, str(color_data.get(pk_entry, def_val))
                    )

            scale_pks_map = {"circularity": 0.7, "solidity": 0.7}
            if color_name in ["red", "blue"]:
                scale_pks_map["cluster_min_aspect_ratio"] = default_color_data.get(
                    "cluster_min_aspect_ratio", 0.2
                )
                scale_pks_map["cluster_max_aspect_ratio"] = default_color_data.get(
                    "cluster_max_aspect_ratio", 3.0
                )
            for pk_scale, def_val_shape in scale_pks_map.items():
                if (
                    f"{pk_scale}_scale" in self.color_vars[color_name]
                    and self.color_vars[color_name][f"{pk_scale}_scale"].winfo_exists()
                ):
                    current_val = color_data.get(pk_scale, def_val_shape)
                    self.color_vars[color_name][f"{pk_scale}_scale"].set(
                        int(current_val * 100)
                    )
    ...
    def generate_mask_for_color(self, color_name, image_to_process):
        ...
        if not hsv_ranges:
            default_ranges = self.main_app.DEFAULT_DETECTION_PARAMS["colors"][
                color_name
            ].get("hsv_ranges", [])
        ...
        if blur_k < 3:
            blur_k = 3
        if blur_k % 2 == 0:
            blur_k = max(3, blur_k - 1 if blur_k > 0 else 3)
        ...
        for lower_hsv_np, upper_hsv_np in hsv_ranges:
            temp_lower, temp_upper = np.array(lower_hsv_np, dtype=np.uint8), np.array(
                upper_hsv_np, dtype=np.uint8
            )
        ...
        op_k = params.get("morph_open_k", def_params.get("morph_open_k", 5))
        op_k = max(3, op_k + (1 if op_k % 2 == 0 else 0))
        op_i = params.get("morph_open_iter", def_params.get("morph_open_iter", 1))
        cl_k = params.get("morph_close_k", def_params.get("morph_close_k", 5))
        cl_k = max(3, cl_k + (1 if cl_k % 2 == 0 else 0))
        cl_i = params.get(
            "morph_close_iter",
            def_params.get("morph_close_iter", 2 if color_name == "white" else 1),
        )
        ...
        if op_i > 0:
            morphed = cv2.morphologyEx(
                morphed,
                cv2.MORPH_OPEN,
                np.ones((op_k, op_k), np.uint8),
                iterations=op_i,
            )
        if color_name in ["red", "blue"]:
            di_k = params.get("morph_dilate_k", def_params.get("morph_dilate_k", 5))
            di_k = max(3, di_k + (1 if di_k % 2 == 0 else 0))
            di_i = params.get(
                "morph_dilate_iter", def_params.get("morph_dilate_iter", 1)
            )
            if di_i > 0:
                morphed = cv2.dilate(
                    morphed, np.ones((di_k, di_k), np.uint8), iterations=di_i
                )
        if cl_i > 0:
            morphed = cv2.morphologyEx(
                morphed,
                cv2.MORPH_CLOSE,
                np.ones((cl_k, cl_k), np.uint8),
                iterations=cl_i,
            )
        return morphed
    ...
    def refresh_mask_display(self, color_name):
        ...
        try:
            ...
            pass  # Avoid error if window closing
    ...
    def reset_current_tab_to_defaults(self):
        try:
            ...
            if (
                ...
                if color_name == "white":
                    self.refresh_all_mask_displays()  # If global param changed
...
class FieldMeasureApp:
    ...
    DEFAULT_DETECTION_PARAMS = {
        ...
        "nms_overlap_threshold": 0.3,  # Adjusted NMS overlap
    }
    ...
    def __init__(self, root_tk):
        ...
        self.distance_display_str = StringVar(value="Dist: N/A")
        ...
        self.red_team_xy_var = StringVar(value="X:-, Y:-")
    ...
    def _update_plc_gui_status(
        self, status_text, lamp_color
    ):  # Existing method for context
        if (
            hasattr(self, "plc_status_label_widget")
            and self.plc_status_label_widget.winfo_exists()
        ):
    ...
    def _attempt_connect_plc(self):
        ...
        try:
            if self.plc_connected:
                self.pymc3e.close()  # Close if already connected before reconnect
    ...
    def create_widgets(self):
        ...
        self.combined_mask_label = Label(
            info_display_frame, bg="black", width=self.LOUPE_DIM, height=self.LOUPE_DIM
        )
        ...
        f_red = LabelFrame(self.control_frame, text="Red Team Cmd", font=sfont_bold)
        ...
        self.corner_listbox = Listbox(f_corners, height=4, font=sfont_small)
        ...
        Button(
            f_detect,
            text="Detect Balls",
            command=lambda: self.run_full_detection_cycle(False),
            font=sfont,
        ).pack(fill=X, pady=1, ipady=0)
        ...
        Button(
            ...
            font=sfont,
        ).grid(row=2, column=0, columnspan=2, pady=1, sticky="ew", ipady=0)
        Button(
            f_target,
            ...
            bg="lightgreen",
            font=sfont_bold,
        ).grid(row=3, column=0, columnspan=2, pady=1, sticky="ew", ipady=0)
        self.distance_display_label = Label(
            f_target, textvariable=self.distance_display_str, font=("Arial", 9, "bold")
        )
        self.distance_display_label.grid(
            row=4, column=0, columnspan=2, pady=0, sticky="w"
        )
        self.angle_display_label = Label(
            f_target, textvariable=self.angle_display_str, font=("Arial", 9, "bold")
        )
        self.angle_display_label.grid(row=5, column=0, columnspan=2, pady=0, sticky="w")
        ...
        f_plc.pack(
            fill=X, padx=2, pady=(5, 1), side=BOTTOM
        )  # Pack at bottom of control_frame
    ...
    def handle_canvas_click(self, event):
        if getattr(self, "selecting_red_team_point", False):
            ...
            self.red_team_xy_var.set(f"X:{X}, Y:{Y}")
            self.selecting_red_team_point = False
    ...
    def update_camera_feed(self):
        ...
        detection_interval = (
            0.05  # Run detection roughly every 50ms (20 FPS max for detection)
        )
        ...
        try:
            while self.running:
                ...
                if ret and frame is not None:
                    ...
                    try:
                        if self.plc_connected and self.pymc3e:
                            read_values = self.pymc3e.batchread_wordunits(
                            ...
                            if m300_value == 3:  # Assuming 3 means Red Team
                                ...
                                self.current_team = (
                                    self.TEAM_BLUE
                                )  # Example: M300 not 3 could mean blue
                    ...
                    if (current_time - last_detection_time) > detection_interval:
                        ...
                        try:
                            ...
                            self.ball_all = (
                                []
                            )  # Clear detections to avoid using stale/bad data
                            self.ball_detected = False
                            self.ball_pt1 = None
                            # Optionally update ball status string here if needed
                            if hasattr(self.root, "after") and self.root.winfo_exists():
                                self.root.after(
                                    0, self.ball_status_str.set, "Ball: Detect Error"
                                )
                        ...
                        if len(self.field_pts) == 4:  # Ensure field is defined
                            if (
                                ...
                                if measurement_data:
                                    ...
                                    final_swing_speed = (
                                        final_dist_cm * 24.096
                                    ) + 5900  # Default calculation
                                    ...
                                    jack_cm_coords = (
                                        measurement_data["x1_cm"],
                                        measurement_data["y1_cm"],
                                    )
                                    target_cm_coords = (
                                        measurement_data["x2_cm"],
                                        measurement_data["y2_cm"],
                                    )
                                    ...
                                    if H_matrix_local is not None:
                                        for (
                                            ...
                                            self.ball_all
                                        ):  # self.ball_all is updated by detect_balls_in_frame
                                            is_opponent = (
                                                self.current_team == self.TEAM_RED
                                    ...
                                    if obstacle_on_path:
                                        final_swing_speed = (
                                            (final_dist_cm * 31.363) + 10864 - 1200
                                        )
                                    ...
                                    if self.plc_connected and self.pymc3e:
                                        ...
                                        try:
                                            self.pymc3e.batchwrite_bitunits(
                                                headdevice="M190",
                                                values=[m190_value_to_set],
                                            )
                                        except Exception as e_plc_m190:
                                            print(
                                                f"ERROR writing M190 to PLC (CameraFeed): {e_plc_m190}"
                                            )

                                    display_data = measurement_data.copy()
                                    display_data["distance_cm_final"] = (
                                        self.last_known_plc_distance
                                    )
                                    display_data["angle_degrees_final"] = (
                                        self.last_known_plc_angle
                                    )
                                    if (
                                ...
                                if (
                                    ...
                                    if (
                                        self.plc_connected and self.pymc3e
                                    ):  # Check pymc3e exists
                                        self.send_data_to_plc(
                                            self.last_known_plc_distance,
                                            self.last_known_plc_angle,
                                        ...
                                        try:
                                            ...
                                            print(
                                                f"ERROR writing M190 OFF to PLC (Jack Disappeared): {e_plc_m190_off}"
                                            )
                ...
                time.sleep(
                    0.005
                )  # Small delay to yield CPU, adjust as needed for performance
    ...
    def run_full_detection_cycle(self, show_results_window=False):
        if self.img is None:
            if (
                not show_results_window
            ):  # Avoid redundant message if detail window was intended
        ...
        if len(self.field_pts) == 4:
            if (
                ...
                if (
                    ...
                    self.plc_connected and self.pymc3e
                ):  # Ensure M190 is off if no valid detection
                    try:
        ...
        if best_hsv_white_ball_for_primary:
            ...
            for color_name in ["red", "blue"]:
                ...
                for cnt_rb in contours_rb:
                    ...
                    if (
                        not is_single_ball and c_min_a <= area_rb <= c_max_a
                    ):  # Check for cluster
                        x_br, y_br, w_br, h_br = cv2.boundingRect(cnt_rb)
                        aspect_ratio_br = (
                            float(w_br) / h_br if h_br > 0 else 0.0
                        )  # Avoid div by zero
        ...
        if self.ball_detected and best_hsv_white_ball_for_primary:
            ...
            if not is_pri_in_list:
                for (
                    ...
                    self.ball_all
                ):  # Ensure no other white ball is accidentally marked primary
    ...
    def update_main_canvas_display(self):
        if self.preview is None:
            if hasattr(self.canvas, "winfo_exists") and self.canvas.winfo_exists():
                ...
                if cw <= 1 or ch <= 1:
                    cw, ch = int(self.canvas.cget("width")), int(
                        self.canvas.cget("height")
                    )
                if cw > 1 and ch > 1:
                    self.canvas.create_text(
                        cw // 2, ch // 2, text="No image / Cam off", font=("Arial", 14)
                    )
            if (
        ...
        for det_obj in self.ball_all:
            ...
            if shape_type == "circle":
                ...
                cv2.rectangle(
                    draw_preview, (x1_p, y1_p), (x2_p, y2_p), draw_clr, 1
                )  # Simple rectangle
    ...
    def start_red_team_select_mode(self):
        ...
        messagebox.showinfo(
            "Red Team Cmd", "Click on image to select Red Team target point."
        )
    ...
    def red_team(self):
        if not self.pymc3e or not self.plc_connected:
            messagebox.showerror(
                "PLC Error", "PLC not connected. Cannot send Red Team command."
            )
            return
        if not self.red_team_selected_point:
            messagebox.showwarning(
                "Red Team Cmd", "Red Team target point not selected on image."
            )
            return
        if len(self.field_pts) != 4:
            messagebox.showwarning(
                "Red Team Cmd", "Field corners not defined. Cannot transform point."
            )
            return
        ...
        try:
            ...
            if H_matrix is None:
                raise ValueError("Failed to get perspective transform.")
            ...
            if rt_cm_transformed is None or rt_cm_transformed.size == 0:
                raise ValueError("Failed to transform Red Team point.")
            ...
            print(
                f"Red Team Cmd: Dist Robot to RT_Point: {distance_to_rt_pt:.1f} cm, Angle: {angle_to_rt_pt_deg:.1f} deg"
            )
            print(
                f"Sending to PLC - Angle (D120): {angle_to_rt_pt_deg:.0f}, Swing (D106): {swing_plc:.0f}, Release (D108): {release_plc:.0f}"
            )
            ...
            messagebox.showinfo(
                "Red Team Cmd",
                f"Red Team command sent to PLC. Angle:{angle_to_rt_pt_deg:.1f}, Swing:{swing_plc:.0f}",
            )

        except ValueError as ve:
            messagebox.showerror("Red Team Cmd Error", f"Input error: {str(ve)}")
        except Exception as e_rt:
            messagebox.showerror(
                "Red Team Cmd Error", f"Failed to send Red Team command: {str(e_rt)}"
            )
            traceback.print_exc()
            self._update_plc_gui_status("Red Cmd Fail", "orange")
