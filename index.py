...
class DraggableDot:
    def __init__(self, canvas, x, y, color, radius=7):
        ...
        self.radius = radius
        self.color = color
        self.dot = canvas.create_oval(x-radius, y-radius, x+radius, y+radius,
                                      fill=color, outline="black", tags="dot")
...
class ImageWithDotsApp:
    ...
    def draw_lines_between_dots(self):
        ...
        distance_y = None
        distance_y = self.calculate_fomula(self.dots["dot1"], self.dots["dot3"]) 
        # + self.calculate_fomula(self.dots["dot2"], self.dots["dot4"])
        ...
        if closest_below:
            ...
            print(f"Formula Result (Closest Below): {formula_result+2.5:.6f} m")
            self.text_show = tk.Label(self.root, text=f"Formula Result : {formula_result+2.5:.6f} m", bg="white", fg="black")
            self.text_show.pack(pady=10)
    ...
    def calculate_distances(self):
        d = self.dots
        self.ClearLines()
        self.draw_lines_between_dots()
        # Print all dot positions
        for name, dot in d.items():
            x, y = self.get_center(dot)
            print(f"{name}: ({x:.2f}, {y:.2f})")

        # คำนวณระยะทางจริงระหว่างจุดที่รู้
        dist_1_2 = self.calculate_distance(d["dot1"], d["dot2"])
        dist_1_3 = self.calculate_distance(d["dot1"], d["dot3"])
        dist_3_4 = self.calculate_distance(d["dot3"], d["dot4"])
        dist_5_1 = self.calculate_distance(d["dot5"], d["dot1"])
        dist_5_2 = self.calculate_distance(d["dot5"], d["dot2"])
        dist_5_3 = self.calculate_distance(d["dot5"], d["dot3"])
        dist_5_4 = self.calculate_distance(d["dot5"], d["dot4"])

        print("\nDistance from dot5 to others:")
        print(f"Dot5 - Dot1: {dist_5_1:.2f} px")
        print(f"Dot5 - Dot2: {dist_5_2:.2f} px")
        print(f"Dot5 - Dot3: {dist_5_3:.2f} px")
        print(f"Dot5 - Dot4: {dist_5_4:.2f} px")

        # สมมติว่าเรารู้ระยะจริงในหน่วยเมตร
        real_d1_d2 = 4.00
        real_d1_d3 = 5.00
        real_d3_d4 = 4.00

        scale_1_2 = real_d1_d2 / dist_1_2
        scale_1_3 = real_d1_d3 / dist_1_3
        scale_3_4 = real_d3_d4 / dist_3_4

        print("\nScales (meters/pixel):")
        print(f"Dot1-Dot2: {scale_1_2:.6f}")
        print(f"Dot1-Dot3: {scale_1_3:.6f}")
        print(f"Dot3-Dot4: {scale_3_4:.6f}")

        avg_scale = (scale_1_2 + scale_1_3 + scale_3_4) / 3
        print(f"Average scale: {avg_scale:.6f} m/px")

if __name__ == "__main__":
    image_path = "/Users/nattavipolboonsangchaitawat/Documents/lab/plc/img/S__8413200.jpg"
    if not os.path.exists(image_path):
        ...
        with Image.open(image_path) as img:
            ...
            resized_image_path = "/Users/nattavipolboonsangchaitawat/Documents/lab/plc/img/image.png"
