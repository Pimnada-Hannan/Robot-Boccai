...
from PIL import (
    ...
    ImageTk,
    ImageColor,
)  # Added ImageColor for safety, though not strictly needed for "lightgrey"
import threading
import traceback
import time
import copy
...
# --- CalibrationWindow Class (Code from 5_23_7.py - Unchanged by current request) ---
class CalibrationWindow(Toplevel):
    ...
    def _auto_apply_to_main_app(self):
        ...
        if self.main_app.img is not None:
            self.main_app.current_hsv_combined_mask_display = (
                self.main_app._generate_hsv_combined_mask_for_display()
            )
    ...
    def create_widgets(self):
        ...
        for color_name in colors_to_calibrate:
            color_specific_settings_frame = LabelFrame(
                settings_container_frame,
                text=f"{color_name.capitalize()} HSV Settings",
                padx=5,
                pady=5,
            )  # Title changed
    ...
    def setup_color_controls(self, parent_frame, color_name):
        ...
        hsv_frame.pack(
            fill=X, pady=2
        )  # Only HSV frame now directly in color_specific_settings_frame
        ...
        for i, (text, min_val, max_val) in enumerate(hsv_params):
            ...
            Label(hsv_frame, text=text + ":").grid(
                row=r, column=c, sticky=W, padx=1, pady=1
            )
            scale = Scale(
                hsv_frame,
                from_=min_val,
                ...
                orient=HORIZONTAL,
                length=120,
                command=lambda val, cn=color_name, p_idx=i: self.update_hsv_param_and_auto_apply(
                    cn, p_idx, int(val)
                ),
            )
    ...
    def update_hsv_param_and_auto_apply(
        self, color_name, param_index_flat, value
    ):  # This remains for HSV
        if not self.calib_params["colors"][color_name].get("hsv_ranges"):
            ...
            self.calib_params["colors"][color_name]["hsv_ranges"] = [
                (default_hsv_lower.copy(), default_hsv_upper.copy())
            ]
            ...
            self.calib_params["colors"][color_name]["hsv_ranges"][0] = (
                default_hsv_lower.copy(),
                default_hsv_upper.copy(),
            )
    ...
    def load_params_to_ui(self):
        ...
        for color_name in ["white", "red", "blue"]:
            ...
            default_color_data = self.main_app.DEFAULT_DETECTION_PARAMS["colors"][
                color_name
            ]

            # HSV Scales loading
            if self.color_vars.get(color_name) and self.color_vars[color_name].get(
                "hsv_scales"
            ):
                required_hsv_scales = [
                    "h_min",
                ...
                if not all(
                    s_key in self.color_vars[color_name]["hsv_scales"]
                    and self.color_vars[color_name]["hsv_scales"][s_key].winfo_exists()
                    for s_key in required_hsv_scales
                ):
                    pass
                else:
                    if (
                        not color_data.get("hsv_ranges")
                        or not color_data["hsv_ranges"][0]
                    ):
                        def_lower, def_upper = default_color_data["hsv_ranges"][0]
                        color_data["hsv_ranges"] = [
                            (def_lower.copy(), def_upper.copy())
                        ]
...
class FieldMeasureApp:
    ...
    DEFAULT_DETECTION_PARAMS = {
        ...
        "colors": {
            "white": {
                "hsv_ranges": [(np.array([0, 0, 180]), np.array([190, 65, 255]))],
                "circularity": 0.65,
                "solidity": 0.75,
                "area_min": 40,
                "area_max": 1500,
            ...
            "red": {
                ...
                "morph_close_k": 5,
                "morph_close_iter": 1,
                "cluster_area_min": 600,
                "cluster_area_max": 6000,
                "cluster_min_aspect_ratio": 0.15,
                "cluster_max_aspect_ratio": 6.0,
            },
            "blue": {
    ...
    OBSTACLE_PROXIMITY_THRESHOLD_CM = 5.0
    ...
    def __init__(self, root_tk):
        ...
        self.target_x_cm_str = StringVar(value="146.0")
        self.target_y_cm_str = StringVar(value="797.5")
        self.pulse_offset_var = StringVar(value="0")  # Added for pulse offset
        self.distance_display_str = StringVar(value="Distance: N/A")
        self.angle_display_str = StringVar(value="Angle: N/A")
        self.swing_speed_display_str = StringVar(value="Swing speed: N/A")
        self.release_position_display_str = StringVar(value="Release pos.: N/A")
        self.ball_status_str = StringVar(value="Ball: Not found")
        ...
        self.white_solidity_var = IntVar(value=int(wp["solidity"] * 100))
        self.white_circularity_var = IntVar(value=int(wp["circularity"] * 100))
        self.white_min_radius_var = IntVar(value=wp.get("primary_min_radius", 6))
        self.current_hsv_combined_mask_display = np.zeros(
            (self.COMBINDED_MASK_DIM, self.COMBINDED_MASK_DIM, 3), dtype=np.uint8
        )
        ...
        try:
            ...
            for color_name_key, color_info in colors_to_draw_ordered:
                ...
                for lower_hsv_np, upper_hsv_np in hsv_ranges_list:
                    ...
                    if not (
                        color_info["params_key"] == "red"
                        and temp_lower[0] > temp_upper[0]
                    ):
    ...
    def _attempt_connect_plc(self):
        ...
        try:
            ...
            self.plc_connected = False
            self._update_plc_gui_status("Failed", "red")
            return False
        finally:
            self.plc_connecting = False
    ...
    def create_widgets(self):
        ...
        _blank_l_img = Image.new(
            "RGB", (self.COMBINDED_MASK_DIM, self.COMBINDED_MASK_DIM), "black"
        )
        ...
        self.coord_label = Label(
            self.image_frame, text="Cursor: X: -, Y: -", font=("Arial", 10)
        )
        self.coord_label.pack(pady=1, side=BOTTOM, fill=X)
        sfont = ("Arial", 10)
        sfont_bold = ("Arial", 10, "bold")
        sfont_small = ("Arial", 9)
        ...
        self.plc_lamp_indicator = self.plc_lamp_canvas.create_oval(
            2, 2, 16, 16, fill="grey", outline="black"
        )
        self.plc_status_label_widget = Label(
            f_plc_inner, text="PLC: Init...", font=sfont
        )
        ...
        )  # This will place it after f_target because f_target was packed first
        self.loupe_label = Label(
            f_loupe, bg="lightgrey", width=self.LOUPE_DIM, height=self.LOUPE_DIM
        )  #
        ...
        f_input = LabelFrame(self.control_frame, text="Input", font=sfont_bold)
        f_input.pack(fill=X, padx=2, pady=1)
        Button(
            f_input, text="Open Image", command=self.open_image_file, font=sfont
        ).pack(fill=X, pady=1, ipady=0)
        Button(f_input, text="Open Camera", command=self.open_camera, font=sfont).pack(
            fill=X, pady=1, ipady=0
        )
        ...
        f_red = LabelFrame(
            self.control_frame, text="Red Team Jack Cmd", font=sfont_bold
        )
        f_red.pack(fill=X, padx=2, pady=1)
        Button(
            f_red,
            text="Select Red Jack Target",
            command=self.start_red_team_select_mode,
            bg="#FFCCCC",
            font=sfont,
        ).pack(fill=X, pady=1, ipady=0)
        self.red_team_xy_label = Label(
            f_red, textvariable=self.red_team_xy_var, font=sfont_bold, bg="white"
        )
        self.red_team_xy_label.pack(fill=X, pady=1)
        Button(
            f_red,
            text="Send Red Jack Command",
            command=self.red_team,
            bg="#FFCCCC",
            font=sfont,
        ).pack(fill=X, pady=1, ipady=0)

        f_corners = LabelFrame(
            self.control_frame, text="Field Corners", font=sfont_bold
        )
        f_corners.pack(fill=X, padx=2, pady=1)
        Label(f_corners, text="Order: TL,BL,TR,BR", font=("Arial", 7, "italic")).pack(
            fill=X
        )
        self.corner_listbox = Listbox(f_corners, height=2, font=("Arial", 7, "italic"))
        self.corner_listbox.pack(fill=X, pady=1)
        cf_btns = Frame(f_corners)
        cf_btns.pack(fill=X)
        Button(
            cf_btns, text="Del Last", command=self.remove_last_point, font=sfont
        ).pack(side=LEFT, expand=True, fill=X, padx=1, ipady=0)
        Button(cf_btns, text="Clear All", command=self.clear_points, font=sfont).pack(
            side=LEFT, expand=True, fill=X, padx=1, ipady=0
        )
        f_detect = LabelFrame(self.control_frame, text="Detection", font=sfont_bold)
        f_detect.pack(fill=X, padx=2, pady=1)
        ...
        self.ball_status_label = Label(
            f_detect, textvariable=self.ball_status_str, font=sfont
        )
        self.ball_status_label.pack(fill=X, pady=0)
        f_pick = Frame(f_detect)
        f_pick.pack(fill=X)
        Button(
            f_pick,
            text="Pick W",
        ...
        f_patch = Frame(f_detect)
        f_patch.pack(fill=X, pady=(1, 0))
        Label(f_patch, text="Pick Patch(px):", font=sfont_small).pack(
            side=LEFT, padx=(0, 2)
        )
        ...
        f_wpri = LabelFrame(
            self.control_frame, text="Primary White (UI)", font=sfont_bold
        )
        f_wpri.pack(fill=X, padx=2, pady=1)
        Label(f_wpri, text="Solidity(%):", font=sfont_small).grid(
        ...
        Label(f_wpri, text="Circularity(%):", font=sfont_small).grid(
        ...
        self.white_circularity_scale = Scale(
            ...
            to=100,
            orient=HORIZONTAL,
            var=self.white_circularity_var,
            command=self.update_white_ball_detection_params_from_main_ui,
            length=60,
            font=sfont_small,
        )
        self.white_circularity_scale.grid(row=1, column=1, sticky=EW, pady=0)
        Label(f_wpri, text="Min Radius(px):", font=sfont_small).grid(
        ...
        f_target = LabelFrame(
            self.control_frame, text="Target & Measure", font=sfont_bold
        )
        f_target.pack(fill=X, padx=2, pady=1)
        Label(f_target, text="Target X (cm):", font=sfont_small).grid(
            row=0, column=0, sticky=W, padx=2
        )
        self.x_entry = Entry(
            f_target, width=7, textvariable=self.target_x_cm_str, font=sfont_small
        )
        self.x_entry.grid(row=0, column=1, sticky=W, padx=2)
        Label(f_target, text="Target Y (cm):", font=sfont_small).grid(
            row=1, column=0, sticky=W, padx=2
        )
        self.y_entry = Entry(
            f_target, width=7, textvariable=self.target_y_cm_str, font=sfont_small
        )
        self.y_entry.grid(row=1, column=1, sticky=W, padx=2)

        # MODIFIED: Added Pulse Offset Entry
        Label(f_target, text="Pulse Offset:", font=sfont_small).grid(
            row=0, column=2, sticky=W, padx=2
        )
        Entry(
            f_target, width=7, textvariable=self.pulse_offset_var, font=sfont_small
        ...
        Button(
            ...
            row=3, column=0, columnspan=2, pady=1, sticky="ew", ipady=0
        )  # Was row 2
        Button(
            f_target,
            text="Calc & Show Detail",
            command=lambda: self.run_full_detection_cycle(True),
            bg="lightgreen",
            font=sfont_bold,
        ).grid(row=3, column=2, columnspan=2, pady=1, sticky="ew", ipady=0)
        self.distance_display_label = Label(
            f_target, textvariable=self.distance_display_str, font=("Arial", 10, "bold")
        )
        self.distance_display_label.grid(
            row=5, column=0, columnspan=2, pady=0, sticky="w"
        )
        self.angle_display_label = Label(
            f_target, textvariable=self.angle_display_str, font=("Arial", 10, "bold")
        )
        self.angle_display_label.grid(row=5, column=2, columnspan=2, pady=0, sticky="w")
        self.swing_speed_display_label = Label(
            f_target,
            textvariable=self.swing_speed_display_str,
            font=("Arial", 10, "bold"),
        )
        self.swing_speed_display_label.grid(
            row=6, column=0, columnspan=2, pady=0, sticky="w"
        )
        self.release_position_display_label = Label(
            f_target,
            textvariable=self.release_position_display_str,
            font=("Arial", 10, "bold"),
        )
        self.release_position_display_label.grid(
            row=6, column=2, columnspan=2, pady=0, sticky="w"
        )
    ...
    def _is_opponent_near_jack(self, jack_cm, opponent_balls_cm_coords, threshold_cm):
        ...
        print(
            # f"DEBUG: _is_opponent_near_jack - Called with: jack_cm={jack_cm}, opponents={opponent_balls_cm_coords}, threshold={threshold_cm}"
        )
    ...
    def _is_obstacle_on_path(
        self, jack_cm, target_cm, obstacle_balls_cm_coords, proximity_threshold_cm
    ):
        # Function from 5_23_7.py
        ...
        for i, (obs_x, obs_y) in enumerate(obstacle_balls_cm_coords):
            ...
            distance_to_line = (
                abs(line_A * obs_x + line_B * obs_y + line_C) / denominator
            )
    ...
    def update_white_ball_detection_params_from_main_ui(self, event=None):
        if "white" in self.active_detection_params["colors"]:
            ...
            if self.img is not None and not self.camera_mode:
                self.run_full_detection_cycle(False)
    ...
    def update_main_ui_detection_params_display(self):
        if hasattr(self, "white_solidity_var"):
            ...
            self.white_solidity_var.set(
                int(act_w.get("solidity", def_w["solidity"]) * 100)
            )
            self.white_circularity_var.set(
                int(act_w.get("circularity", def_w["circularity"]) * 100)
            )
            self.white_min_radius_var.set(
                act_w.get("primary_min_radius", def_w["primary_min_radius"])
            )

    def toggle_info_color_pick_mode(
        self,
    ):  # Method kept even if button removed, in case of other uses
        if self.img is None:
            messagebox.showwarning("Pick Color", "Open image/camera first.")
            return
        ...
        if (
            self.STATUS_CLICK_COLOR_INFO_MODE
            and getattr(self, "picking_hsv_for_color", None) is not None
        ):
    ...
    def get_color_info_from_click(self, event):
        ...
        if not (0 <= ix < self.img.shape[1] and 0 <= iy < self.img.shape[0]):
            return
        bgr = self.img[iy, ix]
    ...
    def set_target_position_action(self):
        try:
            ...
            messagebox.showerror("Invalid Input", "Valid numbers for X,Y target.")

    def handle_canvas_click(self, event):
        if getattr(self, "selecting_red_team_point", False):
            X, Y = int(event.x / self.PREVIEW_SCALE), int(event.y / self.PREVIEW_SCALE)
            self.red_team_selected_point = (X, Y)
            self.red_team_xy_var.set(f"Red Jack Target X:{X}, Y:{Y}")
            self.selecting_red_team_point = False
            self.canvas.config(cursor="")
            messagebox.showinfo(
                "Red Jack Target", f"Selected Red Jack Target point: ({X},{Y})"
            )
            return
        if self.STATUS_CLICK_COLOR_INFO_MODE:
            self.get_color_info_from_click(event)
        elif getattr(self, "picking_hsv_for_color", None) is not None:
    ...
    def _dynamic_scale_and_set_preview(self):
        ...
        if not hasattr(self.root, "winfo_exists") or not self.root.winfo_exists():
            return
        self.root.update_idletasks()
        avail_w = self.image_frame.winfo_width()
        avail_h = self.image_frame.winfo_height()
        info_h_est = self.COMBINDED_MASK_DIM + 10
        coord_h_est = 20
        target_h = avail_h - info_h_est - coord_h_est - 5
        target_w = avail_w - 5
        if target_w <= 1:
            target_w = self.canvas.winfo_width()
        if target_h <= 1:
        ...
        if img_w == 0 or img_h == 0:
            self.preview = self.img.copy() if self.img is not None else None
            self.PREVIEW_SCALE = 1.0
            return
        ...
        prev_w = max(1, int(img_w * self.PREVIEW_SCALE))
        prev_h = max(1, int(img_h * self.PREVIEW_SCALE))
        if prev_w > 0 and prev_h > 0:
            self.preview = cv2.resize(
                self.img, (prev_w, prev_h), interpolation=cv2.INTER_AREA
            )
        else:
            self.preview = None
    ...
    def open_camera(self):
        ...
        try:
            ...
            for w, h in [(2560, 1440), (1920, 1080), (1280, 720)]:
                ...
                time.sleep(0.2)
                aw, ah = int(self.cap.get(cv2.CAP_PROP_FRAME_WIDTH)), int(
                    self.cap.get(cv2.CAP_PROP_FRAME_HEIGHT)
                )
                if abs(aw - w) < 10 and abs(ah - h) < 10:
                    print(f"INFO: Cam res set to {aw}x{ah}")
                    res_set = True
                    break
            if not res_set:
                print(
                    f"WARN: Using default cam res: {int(self.cap.get(cv2.CAP_PROP_FRAME_WIDTH))}x{int(self.cap.get(cv2.CAP_PROP_FRAME_HEIGHT))}"
                )
            ...
            if ret and frame is not None:
                self.img = frame.copy()
                if self.img is None or self.img.size == 0:
                    raise ValueError("Invalid initial cam frame.")
                self.running = True
                self._dynamic_scale_and_set_preview()
                self.current_hsv_combined_mask_display = (
                    self._generate_hsv_combined_mask_for_display()
                ...
                raise ValueError("Could not grab initial frame from cam.")
        except Exception as e:
            print(f"ERR opening cam: {e}")
            traceback.print_exc()
            if self.cap:
                self.cap.release()
                self.cap = None
    ...
    def is_in_v_zone(self, x, y):
        # พิกัดของ V-Zone ฝั่งซ้าย
        self.left_v_zone_pts = [
            (50, 300),  # จุดบนซ้าย
            (50, 500),  # จุดล่างซ้าย
            (200, 600),  # จุดล่างขวา
            (200, 200),  # จุดบนขวา
        ...
        # พิกัดของ V-Zone ฝั่งขวา
        self.right_v_zone_pts = [
            (400, 300),  # จุดบนซ้าย
            (400, 500),  # จุดล่างซ้าย
            (550, 600),  # จุดล่างขวา
            (550, 200),  # จุดบนขวา
        ...
        # ตรวจว่าอยู่ในสนามก่อน (field_pts = 4 จุดของสนามที่คลิกไว้)
        if len(self.field_pts) != 4:
            return None  # ยังไม่ได้ตั้งค่าขอบเขตสนาม

        field_zone = np.array(self.field_pts, dtype=np.int32)
        if cv2.pointPolygonTest(field_zone, (x, y), False) < 0:
            return None  # อยู่นอกสนาม ไม่ต้องตรวจ V-Zone

        # อยู่ในสนามแล้ว → ตรวจว่าอยู่ในโซนไหน
        if cv2.pointPolygonTest(np.array(self.left_v_zone_pts), (x, y), False) >= 0:
            return "left"
        elif cv2.pointPolygonTest(np.array(self.right_v_zone_pts), (x, y), False) >= 0:
            return "right"
        else:
            return "field"  # อยู่ในสนามแต่ไม่อยู่ในโซน V
                ...
                if ret and frame is not None:
                    ...
                    try:
                        if self.plc_connected and self.pymc3e:
                            read_m300_data = self.pymc3e.batchread_wordunits(
                                headdevice="M300", readsize=1
                            )
                            if read_m300_data:
                                m300_value = read_m300_data[0]

                            read_d130_data = self.pymc3e.batchread_wordunits(
                                headdevice="D130", readsize=1
                            )
                            if read_d130_data:
                            ...
                            if m300_value != 0:
                                ...
                                m300_value == 0
                            ):  # Assuming M300 = 0 is the other team (e.g., Blue)
                    ...
                    if (current_time - last_detection_time) > detection_interval:
                        ...
                        pulse_offset_val = 0
                        try:
                            if hasattr(self, "pulse_offset_var") and isinstance(
                                self.pulse_offset_var, StringVar
                            ):
                                pulse_offset_val = int(self.pulse_offset_var.get())
                            else:
                                print(
                                    f"Warning: pulse_offset_var not initialized correctly in update_camera_feed. Using 0."
                                )
                        except ValueError:
                            if hasattr(self, "pulse_offset_var") and isinstance(
                                self.pulse_offset_var, StringVar
                                ...
                                print(
                                    f"Warning: Invalid pulse_offset value in update_camera_feed: '{self.pulse_offset_var.get()}'. Using 0."
                                )
                            else:
                                print(
                                    f"Warning: Invalid pulse_offset value (pulse_offset_var not available) in update_camera_feed. Using 0."
                                )
                        ...
                        if self.current_team == self.TEAM_RED and d130_value == 0:
                            ...
                            self.ball_all, self.ball_detected, self.ball_pt1 = (
                                [],
                                False,
                                None,
                            )
                            if hasattr(self.root, "after") and self.root.winfo_exists():
                                self.root.after(
                                    ...
                                    "Ball: Red Jack (D130=0) - Pick Target",
                                )
                        ...
                        if execute_ball_detection:
                            if len(self.field_pts) == 4:
                                try:
                                    ...
                                    # --- เรียกใช้ is_in_v_zone หลัง detect บอล ---
                                    if self.ball_pt1 is not None:
                                        x, y = self.ball_pt1
                                        v_zone_result = self.is_in_v_zone(x, y)
                                        print(f"Jack อยู่ในโซน: {v_zone_result}")
                                except Exception as e_detect:
                                    print(
                                        f"ERROR in detect_balls_in_frame during camera feed: {e_detect}"
                                    )
                                    traceback.print_exc()
                                    self.ball_all, self.ball_detected, self.ball_pt1 = (
                                        [],
                        ...
                        if not (self.current_team == self.TEAM_RED and d130_value == 0):
                            if len(self.field_pts) == 4:
                                ...
                                current_jack_is_detected_visually = (
                                    self.ball_detected and self.ball_pt1 is not None
                                )
                                ...
                                if current_jack_is_detected_visually:
                                    ...
                                    if measurement_data_current:
                                        jack_cm_for_eval = (
                                            measurement_data_current["x1_cm"],
                                            measurement_data_current["y1_cm"],
                                        )
                                        dist_for_eval = measurement_data_current[
                                            "distance_cm"
                                        ]
                                        angle_for_eval = measurement_data_current[
                                            "angle_degrees"
                                        ]

                                        self.last_known_jack_cm_coords = (
                                            jack_cm_for_eval
                                        )
                                ...
                                if (
                                    not current_jack_is_detected_visually
                                    and self.has_last_known_plc_data
                                    and self.last_known_jack_cm_coords
                                ):
                                ...
                                if (
                                    jack_cm_for_eval
                                    and dist_for_eval is not None
                                    and angle_for_eval is not None
                                ):
                                    target_cm_coords = None
                                    try:
                                        target_cm_coords = (
                                            float(self.target_x_cm_str.get()),
                                            float(self.target_y_cm_str.get()),
                                        )
                                    except ValueError:
                                        if (
                                            hasattr(self.root, "after")
                                        ...
                                        if self.plc_connected and self.pymc3e:
                                            try:  # M190, M191, M192 OFF
                                                self.pymc3e.batchwrite_bitunits(
                                                    headdevice="M190", values=[0, 0, 0]
                                                )
                                            except Exception:
                                                pass
                                        if (
                                            ...
                                            self.root.after(
                                                0,
                                                self.update_main_canvas_display_from_thread,
                                            )
                                        time.sleep(0.005)
                                        continue
                                    ...
                                    try:
                                        current_H_matrix = cv2.getPerspectiveTransform(
                                            src_h, dst_h
                                        )
                                    except Exception:
                                        pass
                                    ...
                                    if current_H_matrix is not None:
                                        for ball_info in self.ball_all:
                                            is_opponent = (
                                                self.current_team == self.TEAM_RED
                                                and ball_info["color_name"] == "blue"
                                            ) or (
                                                self.current_team == self.TEAM_BLUE
                                                and ball_info["color_name"] == "red"
                                            )
                                            if is_opponent:
                                                ball_pixel_center = np.float32(
                                                    [ball_info["center"]]
                                                ).reshape(-1, 1, 2)
                                                ball_cm_transformed = (
                                                ...
                                                if (
                                                    ball_cm_transformed is not None
                                                    and ball_cm_transformed.size > 0
                                                ):
                                                    opponent_balls_cm_in_roi.append(
                                                        tuple(ball_cm_transformed[0, 0])
                                                    )

                                    # --- Strategic conditions ---
                                    obstacle_on_path = self._is_obstacle_on_path(
                                        jack_cm_for_eval,
                                        target_cm_coords,
                                        opponent_balls_cm_in_roi,
                                        self.OBSTACLE_PROXIMITY_THRESHOLD_CM,
                                    )
                                    opponent_near_jack = self._is_opponent_near_jack(
                                        jack_cm_for_eval,
                                        opponent_balls_cm_in_roi,
                                        self.OPPONENT_NEAR_JACK_THRESHOLD_CM,
                                    )
                                    ...
                                    final_swing_speed = (
                                        (dist_for_eval * 24.096)
                                        + 5900
                                        + pulse_offset_val
                                    )  # MODIFIED: Base swing uses pulse_offset_val
                                    final_swing_speed = max(
                                        0, min(final_swing_speed, 23000)
                                    )
                                    ...
                                    if (
                                        opponent_near_jack
                                        or obstacle_on_path
                                        or (not obstacle_on_path)
                                    ):  # ถ้าเข้า 1 2 3 เลือก 3
                                        if opponent_near_jack:  # 3
                                            m190_val = 1
                                            print("Funtion #")

                                            print(
                                                "####################  MORE 2 BALL IN FUNTION 3  ####################"
                                            )
                                            final_swing_speed = (
                                                dist_for_eval * 24.096
                                            ) + 4500
                                            M195 = self.pymc3e.randomwrite(
                                                word_devices=["M195"],
                                                word_values=[
                                                    1
                                                ],  # Assuming m195_val should be 1
                                    ...
                                    elif obstacle_on_path or (
                                        not opponent_near_jack
                                    ):  # ถ้าเข้า 1 2 เลือก 2
                                        if obstacle_on_path:
                                            m191_val = 1
                                            ...
                                            if (
                                                not opponent_near_jack
                                            ):  # only obstacle, NOT opponent near jack
                                    ...
                                    if self.plc_connected and self.pymc3e:
                                        ...
                                        try:
                                            ...
                                            self.pymc3e.batchwrite_bitunits(
                                                headdevice="M190",
                                                values=[m190_val, m191_val, m192_val],
                                            )
                                            print(
                                                f"PLC M-Bits Sent: M190={m190_val}, M191={m191_val}, M192={m192_val}"
                                            )
                                        except Exception as e:
                                            print(
                                                f"ERROR writing M190-M192 (CamFeed): {e}"
                                            )

                                    # display_data = {
                                    #     "distance_cm_final": dist_for_eval,
                                    #     "angle_degrees_final": final_angle_deg,
                                    #     "x1_cm": jack_cm_for_eval[0],
                                    #     "y1_cm": jack_cm_for_eval[1],
                                    #     "x2_cm": target_cm_coords[0],
                                    #     "y2_cm": target_cm_coords[1],
                                    display_data = {
                                        "distance_cm_final": dist_for_eval,
                                        "angle_degrees_final": final_angle_deg,
                                        "x1_cm": jack_cm_for_eval[0],
                                        ...
                                        "x2_cm": target_cm_coords[0],
                                        "y2_cm": target_cm_coords[1],
                                        "swing_speed_final": final_swing_speed,
                                        "release_speed_final": final_release_speed,
                                    }
                                    if (
                                        hasattr(self.root, "after")
                                        and self.root.winfo_exists()
                                    ...
                                    if self.plc_connected and self.pymc3e:
                                        try:  # MODIFIED: M190, M191, M192 OFF
                                            ...
                                            print(
                                                f"PLC M-Bits Cleared (No Jack Eval): M190=0, M191=0, M192=0"
                                            )
                                        except Exception as e:
                                            print(
                                                f"ERROR writing M190-M192 OFF (No Jack Eval): {e}"
                                            )
                                ...
                                if self.plc_connected and self.pymc3e:
                                    try:  # MODIFIED: M190, M191, M192 OFF
                                        ...
                                        print(
                                            f"PLC M-Bits Cleared (No Field): M190=0, M191=0, M192=0"
                                        )
                                    except Exception as e:
                                        print(
                                            f"ERROR writing M190-M192 OFF (No Field): {e}"
                                        )

                        if hasattr(self.root, "after") and self.root.winfo_exists():
                            self.root.after(
                                0, self.update_main_canvas_display_from_thread
                            )
                else:  # Frame not retrieved or invalid
                    current_time_err = time.time()
                    if (
                        current_time_err - last_error_report_time
    ...
    def run_full_detection_cycle(self, show_results_window=False):
        ...
        try:
            if self.plc_connected and self.pymc3e:
                read_m300 = self.pymc3e.batchread_wordunits(
                    headdevice="M300", readsize=1
                )
                if read_m300:
                    m300_value = read_m300[0]

                if m300_value != 0:
                    self.current_team = self.TEAM_RED
                    read_d130_words = self.pymc3e.batchread_wordunits(
                        headdevice="D130", readsize=1
                    )
                    if read_d130_words and read_d130_words[0] == 0:
                        d130_is_zero = True
                elif m300_value == 2:
                    self.current_team = self.TEAM_BLUE
                else:
        ...
        try:
            if hasattr(self, "pulse_offset_var") and isinstance(
                ...
                print(
                    f"Warning: pulse_offset_var not initialized correctly in run_full_detection_cycle. Using 0."
                )
        except ValueError:
            if hasattr(self, "pulse_offset_var") and isinstance(
                self.pulse_offset_var, StringVar
                ...
                print(
                    f"Warning: Invalid pulse_offset value in run_full_detection_cycle: '{self.pulse_offset_var.get()}'. Using 0."
                )
            else:
                print(
                    f"Warning: Invalid pulse_offset value (pulse_offset_var not available) in run_full_detection_cycle. Using 0."
                )
            pass  # Default to 0 if conversion fails
        # --- Get pulse_offset_val --- End MODIFIED
        ...
        if len(self.field_pts) == 4:
            if self.ball_detected and self.ball_pt1 is not None:
                ...
                if measurement_data_dict:
                    ...
                    final_swing_speed = (
                        (final_dist_cm * 24.096) + 5900 + pulse_offset_val
                    )
                    final_release_speed = 800
                    H_matrix_local = measurement_data_dict.get("H_matrix")
                    jack_cm_coords = (
                        measurement_data_dict["x1_cm"],
                        measurement_data_dict["y1_cm"],
                    )
                    target_cm_coords = (
                        measurement_data_dict["x2_cm"],
                        measurement_data_dict["y2_cm"],
                    )
                    opponent_balls_cm_in_roi = []
                    if H_matrix_local is not None:
                        for ball_info in self.ball_all:
                            ...
                            if is_opponent:
                                ...
                                ball_cm_transformed = cv2.perspectiveTransform(
                                    ball_pixel_center, H_matrix_local
                                )
                                if (
                                    ball_cm_transformed is not None
                                    and ball_cm_transformed.size > 0
                    ...
                    if obstacle_on_path:
                        ...
                        if self.current_team == self.TEAM_RED:
                            if final_angle_deg >= 0 and final_angle_deg < 180:
                                final_angle_deg -= 1.0
                            elif final_angle_deg >= 180 and final_angle_deg <= 360:
                                final_angle_deg += 1.0
                    ...
                    if (
                        opponent_near_jack
                    ):  # This formula is different, not affected by pulse_offset
                        # final_swing_speed = (final_dist_cm * 31.363) + 10864 - 1200
                        final_swing_speed = (
                            (final_dist_cm * 24.096) + 5900 + 10000 + pulse_offset_val
                        )
                        final_release_speed = 500
                        m190_value_to_set = 1
                    ...
                    if self.plc_connected and self.pymc3e:
                        ...
                        try:
                            self.pymc3e.batchwrite_bitunits(
                                headdevice="M190", values=[m190_value_to_set]
                            )
                        except Exception as e_plc_m190_fc:
                            print(
                                f"ERROR writing M190 to PLC (FullCycle): {e_plc_m190_fc}"
                            )
                    ...
                    display_data["swing_speed_final"] = self.last_known_plc_swing_speed
                    display_data["release_speed_final"] = (
                        self.last_known_plc_release_speed
                    )
                    ...
                    if self.plc_connected and self.pymc3e:
                        try:
                            self.pymc3e.batchwrite_bitunits(
                                headdevice="M190", values=[0]
                            )
                        except Exception as e:
                            print(f"Error M190 OFF (meas fail): {e}")
            else:
                self.update_measurement_display_default()
                if (
                    perform_visual_detection_full_cycle
                    and self.has_last_known_plc_data
                    ...
                    if self.plc_connected and self.pymc3e:
                        ...
                        try:
                            ...
                            print(f"Error M190 OFF (jack gone): {e}")
                    self.sent_last_data_after_disappearance = True
                elif self.plc_connected and self.pymc3e:
                    try:
                        self.pymc3e.batchwrite_bitunits(headdevice="M190", values=[0])
                    except Exception as e:
                        print(f"Error M190 OFF (no detection): {e}")
        else:
            self.update_measurement_display_default()
            if not self.camera_mode and not show_results_window:
                if hasattr(self, "ball_status_str"):
                    self.ball_status_str.set("Ball: Define 4 field corners")
            if self.plc_connected and self.pymc3e:
                try:
                    self.pymc3e.batchwrite_bitunits(headdevice="M190", values=[0])
                except Exception as e:
                    print(f"Error M190 OFF (no field): {e}")

        if show_results_window:
            if len(self.field_pts) != 4:
                messagebox.showwarning(
                    "Results Error",
                    "4 field corners must be selected for detailed results.",
                )
                return
            if not perform_visual_detection_full_cycle:
                messagebox.showinfo(
                    "Results Info",
                    "Red Team Jack (D130=0). Visual results based on auto-detection not applicable for this specific state.",
                )
                return
            if not self.ball_detected or self.ball_pt1 is None:
                messagebox.showwarning(
                    "Results Error",
                    "Primary white ball not detected. Cannot show details.",
                )
                return
            ...
            if measurement_data_dict:
                ...
                messagebox.showerror(
                    "Results Error", "Could not process measurements for detail view."
                )
    ...
    def detect_balls_in_frame(self):
        ...
        wp = self.active_detection_params["colors"].get(
        ...
        if wp:
            ...
            if hsv_ranges_list:
                ...
                for lr, ur in hsv_ranges_list:
                    l = np.array(lr, dtype=np.uint8)
                    u = np.array(ur, dtype=np.uint8)
                    for ch_idx in range(3):
                        if u[ch_idx] < l[ch_idx]:
                            u[ch_idx] = l[ch_idx]
                    mask_w = cv2.bitwise_or(mask_w, cv2.inRange(hsv_blurred, l, u))
                op_k_w = max(
                    3,
                    wp.get("morph_open_k", 5)
                    + (1 if wp.get("morph_open_k", 5) % 2 == 0 else 0),
                )
                op_i_w = wp.get("morph_open_iter", 1)
                cl_k_w = max(
                    3,
                    wp.get("morph_close_k", 5)
                    + (1 if wp.get("morph_close_k", 5) % 2 == 0 else 0),
                )
                ...
                if op_i_w > 0:
                    mask_w = cv2.morphologyEx(
                        mask_w,
                        cv2.MORPH_OPEN,
                        np.ones((op_k_w, op_k_w), np.uint8),
                        iterations=op_i_w,
                    )
                if cl_i_w > 0:
                    mask_w = cv2.morphologyEx(
                        mask_w,
                        cv2.MORPH_CLOSE,
                        np.ones((cl_k_w, cl_k_w), np.uint8),
                        iterations=cl_i_w,
                    )
                ...
                circ_t_w, sol_t_w = wp.get("circularity", 0.65), wp.get(
                    "solidity", 0.75
                )
                prim_circ_t, prim_rad_t = wp.get("primary_circularity", 0.65), wp.get(
                    "primary_min_radius", 6
                )
                for cnt in contours_w:
                    area = cv2.contourArea(cnt)
                    if not (min_a_w <= area <= max_a_w):
                        continue
        ...
        if best_hsv_white_ball_for_primary:
            ...
            self.ball_pt1 = (
                primary_jack_center_px[0],
                primary_jack_center_px[1] + primary_jack_radius_px,
            )
            self.ball_status_str.set(
                f"W:Found (C:{best_hsv_white_ball_for_primary['circularity']:.2f} R:{primary_jack_radius_px})"
            )
            ROI_RADIUS_FACTOR = 12.0
            search_radius_sq_px = (primary_jack_radius_px * ROI_RADIUS_FACTOR) ** 2
            for color_name in ["red", "blue"]:
                cp = self.active_detection_params["colors"].get(
                    color_name, self.DEFAULT_DETECTION_PARAMS["colors"][color_name]
                )
                hsv_ranges_list_rb = cp.get("hsv_ranges", [])
                if not hsv_ranges_list_rb:
                    continue
                ...
                for lr, ur in hsv_ranges_list_rb:
                    ...
                    if color_name == "red" and l[0] > u[0]:
                        ...
                        seg_mask_rb = cv2.inRange(hsv_blurred, l, u)
                    mask_rb = cv2.bitwise_or(mask_rb, seg_mask_rb)
                op_k_rb = max(
                    3,
                    cp.get("morph_open_k", 5)
                    + (1 if cp.get("morph_open_k", 5) % 2 == 0 else 0),
                )
                op_i_rb = cp.get("morph_open_iter", 1)
                di_k_rb = max(
                    3,
                    cp.get("morph_dilate_k", 5)
                    + (1 if cp.get("morph_dilate_k", 5) % 2 == 0 else 0),
                )
                di_i_rb = cp.get("morph_dilate_iter", 1)
                cl_k_rb = max(
                    3,
                    cp.get("morph_close_k", 5)
                    + (1 if cp.get("morph_close_k", 5) % 2 == 0 else 0),
                )
                cl_i_rb = cp.get("morph_close_iter", 1)
                if op_i_rb > 0:
                    mask_rb = cv2.morphologyEx(
                        mask_rb,
                        cv2.MORPH_OPEN,
                        np.ones((op_k_rb, op_k_rb), np.uint8),
                        iterations=op_i_rb,
                    )
                if di_i_rb > 0:
                    mask_rb = cv2.dilate(
                        mask_rb,
                        np.ones((di_k_rb, di_k_rb), np.uint8),
                        iterations=di_i_rb,
                    )
                if cl_i_rb > 0:
                    mask_rb = cv2.morphologyEx(
                        mask_rb,
                        cv2.MORPH_CLOSE,
                        np.ones((cl_k_rb, cl_k_rb), np.uint8),
                        iterations=cl_i_rb,
                    )
                contours_rb, _ = cv2.findContours(
                    mask_rb, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
                )
                min_a_rb, max_a_rb = cp.get("area_min", 100), cp.get("area_max", 700)
                circ_t_rb, sol_t_rb = cp.get("circularity", 0.6), cp.get(
                    "solidity", 0.6
                )
                c_min_a, c_max_a = cp.get("cluster_area_min", 600), cp.get(
                    "cluster_area_max", 6000
                )
                c_min_ar, c_max_ar = cp.get("cluster_min_aspect_ratio", 0.15), cp.get(
                    "cluster_max_aspect_ratio", 6.0
                )
                for cnt_rb in contours_rb:
                    ...
                    if (cx_rb - primary_jack_center_px[0]) ** 2 + (
                        cy_rb - primary_jack_center_px[1]
                    ) ** 2 > search_radius_sq_px:
                        continue
                    ...
                    if not is_single_ball and c_min_a <= area_rb <= c_max_a:
                        ...
                        if is_valid_aspect:
                            conf_cl = (
                                ...
                                + min(
                                    1.0,
                                    (area_rb - c_min_a) / (c_max_a - c_min_a + 1e-6),
                                )
                                * 0.6
                            )
                            all_detected_objects_for_nms.append(
                                {
                                    "bbox": (x_br, y_br, x_br + w_br, y_br + h_br),
                                    "rect_coords": (x_br, y_br, w_br, h_br),
                                    "confidence": conf_cl,
                                    "center": (cx_rb, cy_rb),
                                    "color_name": color_name,
        ...
        if all_detected_objects_for_nms:
            ...
            for i, obj in enumerate(all_detected_objects_for_nms):
                ...
                confs_nms.append(obj["confidence"])
                orig_indices.append(i)
            if boxes_nms:
                try:
                    nms_idx = cv2.dnn.NMSBoxes(
                        boxes_nms,
                        np.array(confs_nms).astype(np.float32),
                        self.active_detection_params.get(
                            "detection_threshold_nms", 0.01
                        ),
                    ...
                    if isinstance(nms_idx, np.ndarray):
                        ...
                        for idx_val in nms_idx:
                            self.ball_all.append(
                                all_detected_objects_for_nms[orig_indices[idx_val]]
                            )
                except Exception as e_nms:
                    print(f"NMS Error: {e_nms}")
                    self.ball_all = all_detected_objects_for_nms
        if self.ball_detected and best_hsv_white_ball_for_primary:
            is_pri_in_list = any(
                b.get("is_primary_white", False) for b in self.ball_all
            )
            if not is_pri_in_list:
                for b_obj in self.ball_all:
                    if b_obj["color_name"] == "white" and b_obj.get("is_primary_white"):
                        b_obj["is_primary_white"] = False
                self.ball_all.append(best_hsv_white_ball_for_primary)
        if (
            not self.ball_all
            and not self.ball_detected
            and not self.ball_status_str.get().startswith("Ball: Define 4")
        ):
    ...
    def update_main_canvas_display(self):
        ...
        if self.preview is None or self.preview.size == 0:
            ...
            if (
                ...
                placeholder_w = int(
                    target_display_height * default_placeholder_aspect_ratio
                )
                if placeholder_w <= 0:
                    placeholder_w = 1
                ...
                if (
                    ...
                    and self.current_hsv_combined_mask_display.ndim >= 2
                    and self.current_hsv_combined_mask_display.shape[0] > 0
                    and self.current_hsv_combined_mask_display.shape[1] > 0
                ):
                ...
                _blank_mask_img = Image.new(
                    "RGB", (placeholder_w, target_display_height), "black"
                )
        ...
        for i, pt_orig in enumerate(self.field_pts):
            if pt_orig is None or len(pt_orig) != 2:
                continue
            x_prev, y_prev = int(pt_orig[0] * self.PREVIEW_SCALE), int(
                pt_orig[1] * self.PREVIEW_SCALE
            )
        ...
        for det_obj in self.ball_all:
            ...
            if center_orig is None or len(center_orig) != 2:
                continue
            ...
            if shape_type == "circle":
                ...
                if det_obj.get("is_primary_white"):
                    label_txt = "Jack"
                cv2.putText(
                    draw_preview,
                    label_txt,
                    (
                        center_prev[0] - radius_prev // 2,
                        center_prev[1] - radius_prev - 2,
                    ),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.3,
                    draw_clr,
                ...
                if rect_coords and len(rect_coords) == 4:
                    ...
                    x1_p, y1_p = int(x_br * self.PREVIEW_SCALE), int(
                        y_br * self.PREVIEW_SCALE
                    )
                    x2_p, y2_p = int((x_br + w_br) * self.PREVIEW_SCALE), int(
                        (y_br + h_br) * self.PREVIEW_SCALE
                    )
                    cv2.rectangle(draw_preview, (x1_p, y1_p), (x2_p, y2_p), draw_clr, 1)
                    cv2.putText(
                        draw_preview,
                        f"{color_name[0].upper()}Cl",
                        (x1_p, y1_p - 2),
                        cv2.FONT_HERSHEY_SIMPLEX,
                        0.3,
                        draw_clr,
        ...
        if self.ball_detected and self.ball_pt1 and len(self.ball_pt1) == 2:
            p1_mx, p1_my = int(self.ball_pt1[0] * self.PREVIEW_SCALE), int(
        ...
        try:
            img_pil_canvas = Image.fromarray(
                cv2.cvtColor(draw_preview, cv2.COLOR_BGR2RGB)
            )
            ...
            try:
                ...
                if orig_h > 0 and orig_w > 0:
                    ...
                    if target_display_width <= 0:
                        target_display_width = 1

                    mask_disp_resized = cv2.resize(
                        self.current_hsv_combined_mask_display,
                    ...
                    if placeholder_w <= 0:
                        placeholder_w = 1
                    mask_disp_resized = np.zeros(
                        (target_display_height, placeholder_w, 3), dtype=np.uint8
                    )
                ...
                if (
                    ...
                    placeholder_w = int(
                        placeholder_h * default_placeholder_aspect_ratio
                    )
                    if placeholder_w <= 0:
                        placeholder_w = 1
                    _error_mask_img = Image.new(
                        "RGB", (placeholder_w, placeholder_h), "darkred"
                    )
    ...
    def add_point(self, event):
        if self.img is None or self.preview is None:
            messagebox.showwarning("Add Point", "No image.")
            return
        if not (
            0 <= event.x < self.preview.shape[1]
            and 0 <= event.y < self.preview.shape[0]
            ...
            return
        if len(self.field_pts) < 4:
            Xo, Yo = int(event.x / self.PREVIEW_SCALE), int(
                event.y / self.PREVIEW_SCALE
            )
            if not (0 <= Xo < self.img.shape[1] and 0 <= Yo < self.img.shape[0]):
                return
            ...
            messagebox.showinfo(
                "Add Point", "4 corners already set. Clear to reselect."
            )
    ...
    def process_measurements_for_realtime(self):
        ...
        if ball1_cm_t is None or ball1_cm_t.size == 0:
            return None
        x1cm, y1cm = ball1_cm_t[0, 0]
        try:
            x2cm, y2cm = float(self.target_x_cm_str.get()), float(
                self.target_y_cm_str.get()
            )
        except ValueError:
            x2cm, y2cm = 0.0, 0.0
    ...
    def update_measurement_display(self, data_dict):
        if data_dict and isinstance(data_dict, dict):
            ...
            ang_disp = data_dict.get(
                "angle_degrees_final", data_dict.get("angle_degrees")
            )

            swing_speed_disp = data_dict.get("swing_speed_final")
            release_pos_disp = data_dict.get("release_speed_final")

            if dist_disp is not None:
                self.distance_display_str.set(f"Dist: {dist_disp:.1f} cm")
            else:
                self.distance_display_str.set("Distance: Error")

            if ang_disp is not None:
                self.angle_display_str.set(f"Angle: {ang_disp:.1f}°")
            else:
                self.angle_display_str.set("Angle: Error")

            if swing_speed_disp is not None:
                self.swing_speed_display_str.set(f"Swing speed: {swing_speed_disp:.0f}")
            else:
                self.swing_speed_display_str.set("Swing speed: Error")

            if release_pos_disp is not None:
                self.release_position_display_str.set(
                    f"Release pos.: {release_pos_disp:.0f}"
                )
            else:
                self.release_position_display_str.set("Release pos.: Error")

        else:
            self.update_measurement_display_default()

    def update_measurement_display_default(self):
        self.distance_display_str.set("Dist: N/A")
    ...
    def show_detailed_results_window(self, measurement_data):
        if not measurement_data or not isinstance(measurement_data, dict):
            return
        x1, y1, x2, y2, dist, angle, ball1_px, H_mat = (
            measurement_data.get(k)
            for k in [
                "x1_cm",
                "y1_cm",
                "x2_cm",
        ...
        if any(v is None for v in [x1, y1, x2, y2, dist, angle, ball1_px, H_mat]):
            messagebox.showerror("Results Data", "Incomplete data.")
            return
        warp_w, warp_h = int(self.FIELD_W_CM), int(self.FIELD_H_CM)
        max_dim = 500
        scale_f = 1.0
        if warp_w <= 0 or warp_h <= 0:
            messagebox.showerror("Field Err", "Field dims must be positive.")
            return
        if warp_w > max_dim or warp_h > max_dim:
            scale_f = min(max_dim / warp_w, max_dim / warp_h)
        disp_w, disp_h = max(1, int(warp_w * scale_f)), max(1, int(warp_h * scale_f))
        if self.img is None:
            messagebox.showerror("Image Err", "Original image not available.")
            return
        warped_nat = cv2.warpPerspective(self.img, H_mat, (warp_w, warp_h))
        if warped_nat is None or warped_nat.size == 0:
            messagebox.showerror("Warp Err", "Failed to warp image.")
            return
        pt1d = (int(x1 * scale_f), int(y1 * scale_f))
        pt2d = (int(x2 * scale_f), int(y2 * scale_f))
        warped_disp = cv2.resize(
            warped_nat, (disp_w, disp_h), interpolation=cv2.INTER_LINEAR
        )
        ...
        cv2.arrowedLine(
            warped_disp, pt1d, pt2d, (255, 255, 255), max(1, int(1 * scale_f))
        )
        res_win = Toplevel(self.root)
        res_win.title("Measurement Details")
        res_win.grab_set()
        res_win.top_view_photo = ImageTk.PhotoImage(
            Image.fromarray(cv2.cvtColor(warped_disp, cv2.COLOR_BGR2RGB))
        )
        ...
        res_f = Frame(res_win)
        res_f.pack(pady=5, padx=5, fill=X)
        sfont = ("Arial", 9)
        sfont_bold = ("Arial", 9, "bold")
        Label(res_f, text=f"P1 (Jack) Original Pixel: {ball1_px}", font=sfont).pack(
            anchor=W
        )
        Label(
            res_f, text=f"P1 Field Coords (cm): ({x1:.1f}, {y1:.1f})", font=sfont_bold
        ).pack(anchor=W)
        Label(
            res_f, text=f"Target Field Coords (cm): ({x2:.1f}, {y2:.1f})", font=sfont
        ).pack(anchor=W)
        ttk.Separator(res_f, orient="horizontal").pack(fill="x", pady=3)
        Label(res_f, text=f"Distance to Target: {dist:.1f} cm", font=sfont_bold).pack(
            anchor=W
        )
        Label(
            res_f, text=f"Angle to Target (from +Y, CW): {angle:.1f}°", font=sfont_bold
        ).pack(anchor=W)
    ...
    def open_calibration_window(self):
        if self.img is None:
            messagebox.showwarning("Calibration", "Open image/camera first.")
            return
    ...
    def initiate_hsv_color_pick_for_params(self, color_name):
        if self.img is None:
            messagebox.showwarning(
                "Pick Color", f"Open image/camera to pick {color_name}."
            )
            return
        if self.STATUS_CLICK_COLOR_INFO_MODE:
            self.toggle_info_color_pick_mode()  # Will print to console now
        ...
        messagebox.showinfo(
            ...
            f"Click on a {color_name} ball to set its HSV params. Click '{color_name.capitalize()}' button again to cancel.",
        )
    ...
    def process_hsv_color_pick(self, event):
        ...
        if not (
            ...
            messagebox.showwarning("Pick Error", "Clicked outside preview.")
            return
        Xo, Yo = int(event.x / self.PREVIEW_SCALE), int(event.y / self.PREVIEW_SCALE)
        if not (0 <= Xo < self.img.shape[1] and 0 <= Yo < self.img.shape[0]):
            messagebox.showwarning("Pick Error", "Clicked outside original image.")
            return
        patch_sz = self.color_pick_patch_size_var.get()
        patch_sz = max(3, patch_sz + (1 if patch_sz % 2 == 0 else 0))
        half_p = patch_sz // 2
        ...
        if bgr_patch.size == 0:
            messagebox.showwarning("Pick Error", "Selected patch empty.")
            return
        ...
        if color_name_picked == "white":
            ...
            if lh1 < 0:
                ranges.append(
                    (np.array([max(0, 179 + lh1), ls, lv]), np.array([179, us, uv]))
                )
                ranges.append(
                    (np.array([0, ls, lv]), np.array([min(179, uh1), us, uv]))
                )
            elif uh1 > 179:
                ranges.append(
                    (np.array([max(0, lh1), ls, lv]), np.array([179, us, uv]))
                )
                ranges.append(
                    (np.array([0, ls, lv]), np.array([min(179, uh1 - 179), us, uv]))
                )
            else:
                ranges.append(
                    (np.array([max(0, lh1), ls, lv]), np.array([min(179, uh1), us, uv]))
                )
            ...
            ls, us = max(s_min_th, sp - s_del), min(255, sp + s_del)
            lv, uv = max(v_min_th, vp - v_del), min(255, vp + v_del)
            target_params["hsv_ranges"] = [
                (np.array([lh, ls, lv]), np.array([uh, us, uv]))
            ]
        messagebox.showinfo(
            "Pick Success",
            f"HSV for '{color_name_picked}' updated. Open Adv. Calib to fine-tune.",
        )
        if (
            self.calibration_window_open
            and self.calibration_window
    ...
    def update_loupe_and_coords(self, event):
        if self.img is None or self.preview is None or self.preview.size == 0:
            ...
            if (
                ...
                if not hasattr(self, "_blank_loupe_photo_ref"):
                    _b_loupe = Image.new(
                        "RGB", (self.LOUPE_DIM, self.LOUPE_DIM), "lightgrey"
                    )  # LOUPE_DIM is now 240
        ...
        if 0 <= event.x < prev_w and 0 <= event.y < prev_h:
            ...
            if hasattr(self.coord_label, "winfo_exists"):
                self.coord_label.config(text=f"Cursor: X:{event.x}, Y:{event.y}")
            Xo, Yo = int(event.x / self.PREVIEW_SCALE), int(
                event.y / self.PREVIEW_SCALE
            )
            if not (0 <= Xo < self.img.shape[1] and 0 <= Yo < self.img.shape[0]):
                return
            h_loupe_o = int(
                self.LOUPE_DIM / (2 * self.LOUPE_SCALE)
            )  # LOUPE_DIM is now 240
            x1, y1 = max(0, Xo - h_loupe_o), max(0, Yo - h_loupe_o)
            x2, y2 = min(self.img.shape[1], Xo + h_loupe_o), min(
                self.img.shape[0], Yo + h_loupe_o
            )
            ...
            self.loupe_photo = ImageTk.PhotoImage(
                Image.fromarray(cv2.cvtColor(loupe_res, cv2.COLOR_BGR2RGB))
            )
            if (
                hasattr(self.loupe_label, "winfo_exists")
                and self.loupe_label.winfo_exists()
    ...
    def on_closing(self):
        ...
        if self.pymc3e and self.plc_connected:
            try:
                self.pymc3e.close()
            except Exception as e_plc_close:
                print(f"Err closing PLC: {e_plc_close}")
        self.plc_connected = False
        if self.calibration_window_open and self.calibration_window:
            try:
                if self.calibration_window.winfo_exists():
    ...
    def start_red_team_select_mode(self):
        ...
        messagebox.showinfo(
            "Red Jack Target",
            "Click on the image to select the Red Team's Jack ball target point.",
        )
    ...
    def red_team(self):
        if not self.pymc3e or not self.plc_connected:
            messagebox.showwarning(
                "PLC Error", "PLC not connected. Cannot send Red Jack command."
            )
        ...
        try:
            ...
            try:
                if hasattr(self, "pulse_offset_var") and isinstance(
                    ...
                    print(
                        f"Warning: pulse_offset_var not initialized correctly in red_team. Using 0."
                    )
            except ValueError:
                if hasattr(self, "pulse_offset_var") and isinstance(
                    self.pulse_offset_var, StringVar
                    ...
                    print(
                        f"Warning: Invalid pulse_offset value in red_team: '{self.pulse_offset_var.get()}'. Using 0."
                    )
                else:
                    print(
                        f"Warning: Invalid pulse_offset value (pulse_offset_var not available) in red_team. Using 0."
                    )
                pass  # Default to 0 if conversion fails
            # --- Get pulse_offset_val --- End MODIFIED
            ...
            if m300_value != 0:
                print(
                    "PLC indicates RED TEAM mode active for Jack throw (M300 indicates Red)."
                )
                d130_read = self.pymc3e.batchread_wordunits(
                    headdevice="D130", readsize=1
                )
                ...
                if d130_value == 0:
                    print(
                        "PLC indicates ball count is 0 (D130 == 0). Proceeding with Red Jack command."
                    )

                    if not self.red_team_selected_point:
                        messagebox.showerror(
                            "Red Jack Error",
                            "Red Jack target point not selected on the image.",
                        )
                    ...
                    if len(self.field_pts) != 4:
                        messagebox.showerror(
                            "Field Error",
                            "Field corners not defined. Cannot transform point for Red Jack command.",
                        )
                        print(
                            "Field corners not defined. Cannot transform point for Red Jack command."
                        )
                    ...
                    if H_matrix is None:
                        messagebox.showerror(
                            "Transform Error",
                            "Failed to get perspective transform for Red Jack command.",
                        )
                        print(
                            "Failed to get perspective transform for Red Jack command."
                        )
                    ...
                    rt_pixel_np = np.float32([[rt_pixel_x, rt_pixel_y]]).reshape(
                        -1, 1, 2
                    )
                    ...
                    if rt_cm_transformed is None or rt_cm_transformed.size == 0:
                        messagebox.showerror(
                            "Transform Error",
                            "Failed to transform Red Jack selected point to field coordinates.",
                        )
                    ...
                    print(
                        f"Red Jack Picked Point (Pixel): ({rt_pixel_x},{rt_pixel_y}), Transformed to Field (cm): ({x_rt_cm:.1f}, {y_rt_cm:.1f})"
                    )
                    ...
                    try:
                        ...
                        print(
                            f"Robot's Throwing Position (cm from UI): ({robot_x_cm:.1f}, {robot_y_cm:.1f})"
                        )
                    except ValueError:
                        messagebox.showerror(
                            "Input Error",
                            "Invalid robot target (throwing origin) coordinates for Red Jack command.",
                        )
                    ...
                    print(
                        f"Red Jack Cmd Calculation: Dist Robot to PickedPt: {distance_to_rt_pt:.1f} cm, Angle: {angle_to_rt_pt_deg:.1f} deg"
                    )
                    print(
                        f"Values for PLC - Angle(D120): {angle_to_rt_pt_deg:.0f}, Swing(D106): {swing_plc:.0f}, Release(D108): {release_plc:.0f}"
                    )
                    ...
                    print(
                        f"Red Jack command data sent to PLC: Angle={angle_to_rt_pt_deg:.1f}°, Swing={swing_plc:.0f}, Release={release_plc:.0f}"
                    )
                    ...
                    if d130_value != 0:
                        messagebox.showwarning(
                            "Red Jack Info",
                            "Red Team mode active (M300 indicates Red), but ball count (D130) is not 0.",
                        )
                    else:
                        messagebox.showwarning(
                            "Red Jack Info",
                            "Red Team Jack conditions not fully met or PLC read issue.",
                        )
            else:
                print(
                    "Not in Red Team mode for Jack throw (M300 does not indicate Red or M300 read failed). No action taken."
                )
                messagebox.showinfo(
                    "Team Status",
                    "Not in Red Team mode for Jack throw (M300 is not 3).",
                )  # Assuming 3 was the original check logic intent
...
if __name__ == "__main__":
    ...
    root.geometry(
        "1000x920"
    )  # Window size might need adjustment depending on new LOUPE_DIM
